...
Authoring and interchange surface by default. Python runs to *produce* batches; after a batch crosses the C Data Interface, execution is Rust except for plan-declared, first-party pre-contract batch transform hooks admitted by `.10x/decisions/python-in-process-batch-transform-hooks.md`. Those hooks reuse the optional `cdf-python` PyO3 pool and remain outside the kernel; the kernel's dependency graph contains no Python (§9.4). No other downstream Python execution is admitted.
