...
Created: 2026-07-06
Updated: 2026-07-06
Relates-To: .10x/tickets/done/2026-07-06-duckdb-ledger-mirror-doctor-drift.md, .10x/specs/checkpoint-state-firn-line.md, .10x/specs/project-cli-observability-security.md, .10x/specs/destination-receipts-guarantees.md

# DuckDB Ledger Mirror Doctor Drift Evidence

## What was observed

`firn doctor` now replaces the prior unsupported `ledger_destination_drift` check for local `duckdb://` destinations with a concrete read-only probe when the SQLite state database and DuckDB destination database both already exist.
...
- Reads only existing `_firn_loads` and `_firn_state` tables.
- Compares committed local ledger heads with receipts against `_firn_loads` by `(target, idempotency_token)` and `_firn_state` by `(target, package_hash, segment_id)`.
...
cargo fmt --all -- --check
cargo check -p firn-dest-duckdb -p firn-cli --locked
cargo clippy -p firn-dest-duckdb -p firn-cli --all-targets --locked -- -D warnings
...
semgrep scan --config p/rust --error crates/firn-cli/src/doctor_drift.rs crates/firn-dest-duckdb/src/api.rs crates/firn-dest-duckdb/src/mirrors.rs
semgrep scan --config p/security-audit --error crates/firn-cli/src/doctor_drift.rs crates/firn-dest-duckdb/src/api.rs crates/firn-dest-duckdb/src/mirrors.rs
...
CARGO_TARGET_DIR=target/quality/geiger-target cargo geiger --manifest-path /Users/alexanderbut/code_projects/personal/firn/crates/firn-cli/Cargo.toml --all-targets --all-features --include-tests --locked
cargo bloat --release -p firn-cli --bin firn-cli -n 20
```

Results:

- Parent integration reran `cargo test -p firn-dest-duckdb --locked --no-fail-fast`, `cargo test -p firn-cli --locked --no-fail-fast`, and `cargo fmt --all -- --check`; all exited 0 before broader quality checks.
- `cargo test -p firn-dest-duckdb --locked --no-fail-fast` exited 0: 8 tests passed, 0 failed, 0 ignored; doc-tests 0 passed.
- `cargo test -p firn-cli --locked --no-fail-fast` exited 0 after adding the final drift fixture: 16 tests passed, 0 failed, 0 ignored; binary tests 0 passed; doc-tests 0 passed.
- `cargo fmt --all -- --check` exited 0.
- `cargo check -p firn-dest-duckdb -p firn-cli --locked` exited 0.
- `cargo clippy -p firn-dest-duckdb -p firn-cli --all-targets --locked -- -D warnings` exited 0.
...
- `cargo llvm-cov --workspace --all-features --locked --summary-only` exited 0 and reported 73.30% region coverage, 72.60% function coverage, and 76.96% line coverage. `crates/firn-cli/src/doctor_drift.rs` reported 85.82% region coverage, 82.35% function coverage, and 88.95% line coverage.
...
- `cargo bloat --release -p firn-cli --bin firn-cli -n 20` exited 0. The release binary text section is 28.9 MiB, with top symbols dominated by DuckDB/libduckdb. `firn_cli::commands::dispatch` is 43.5 KiB; the drift module does not appear in the top 20 symbols.
...
The no-creation requirement is supported directly by the skipped CLI test, which checks both missing state and destination paths remain absent after `firn doctor`.
