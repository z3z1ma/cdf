...
The rerun wrote `target/quality/reports/mutants-python-doctor-probe-rerun/mutants.out/outcomes.json` and reported 36 total mutants, 33 caught, 1 missed, 2 unviable, and 0 timeouts. The remaining missed mutant is `crates/cdf-cli/src/commands.rs:904:5: replace is_executable -> bool with false`, which mutates the `#[cfg(not(unix))]` fallback branch that is not compiled on this macOS run. The Unix executable check path is covered by `doctor_fails_non_executable_python_interpreter`.
