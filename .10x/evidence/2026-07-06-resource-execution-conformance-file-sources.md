...
- `cargo test -p firn-conformance --locked resource -- --nocapture` passed with 10 resource tests. The output includes expected panic messages from negative self-tests that deliberately prove the harness fails bad resources.
- `cargo test -p firn-formats --locked --no-fail-fast` passed with 6 tests and 0 doctests.
- `cargo clippy -p firn-conformance -p firn-formats --all-targets --locked -- -D warnings` passed.
- `cargo nextest run -p firn-conformance -p firn-formats --locked` passed with 38 tests run, 38 passed, 0 skipped.
- `cargo fmt --all -- --check` passed after formatting the compatibility edit.
- `git diff --check -- . ':(exclude).gitignore'` passed.
- `cargo doc -p firn-conformance -p firn-formats --no-deps --locked` passed.
...
- `cargo mutants --file crates/firn-conformance/src/resource/execution.rs --file crates/firn-formats/src/resource.rs --test-package firn-conformance --test-package firn-formats --cargo-arg --locked --jobs 4 --timeout 180 --output target/quality/mutants-resource-execution-file-final` passed: 35 mutants tested in 10 minutes, 22 caught, 13 unviable, 0 missed.
- `cargo llvm-cov -p firn-conformance -p firn-formats --locked --summary-only` passed. The final summary reported total line coverage 78.75%; `firn-conformance/src/resource/execution.rs` line coverage 98.14%; `firn-formats/src/resource.rs` line coverage 82.09%.
...
- `semgrep scan --config p/rust --error --no-git-ignore --json --output target/quality/reports/semgrep-resource-execution-file.json crates/firn-conformance crates/firn-formats` passed with 0 findings.
...
- A source-only `gitleaks dir` scan over a temporary mirror of `git ls-files --cached --others --exclude-standard -- Cargo.toml Cargo.lock crates/firn-conformance crates/firn-formats` passed with no leaks and wrote `target/quality/reports/gitleaks-resource-execution-file-source.json`.
- `rg -n '\bunsafe\b|extern "|\\*const|\\*mut|unsafe impl|impl (Send|Sync)' crates/firn-conformance crates/firn-formats > target/quality/reports/unsafe-scan-resource-execution-file.txt` exited 1 with no matches, which is the expected no-owned-unsafe result.
- Isolated `cargo geiger` runs for `firn-conformance` and `firn-formats` passed with JSON reports under `target/quality/reports/`. Both owned root packages reported 0 used unsafe functions, expressions, unsafe impls, unsafe traits, and unsafe methods. Geiger stderr contained parse warnings for third-party crate test files (`signal-hook-registry`, `pin-project`), so the direct owned-source unsafe scan remains the primary first-party signal.
- `cargo machete --with-metadata` passed with no unused dependencies.
- `cargo +nightly udeps -p firn-conformance -p firn-formats --all-targets --locked` passed; all dependencies appeared used.
- `cargo semver-checks check-release -p firn-conformance --baseline-rev HEAD` passed.
- `cargo semver-checks check-release -p firn-formats --baseline-rev HEAD` initially found a breaking public-field addition to `FormatRead`; the implementation was changed to avoid adding public fields, and the final rerun passed.
- `rg -n '^name = "(parquet|paste)"|parquet =|paste =' Cargo.lock crates/firn-conformance/Cargo.toml crates/firn-formats/Cargo.toml crates/firn-conformance/src crates/firn-formats/src > target/quality/reports/parquet-paste-scan-resource-execution-file.txt` exited 1 with no matches, confirming this slice did not add the native `parquet`/`paste` advisory path.
...
- Reusable async execution conformance helper exists and is exported from `firn-conformance::resource`.
...
- `firn-formats::FileResource` reuses `read_file_source` and the existing DuckDB-backed Parquet path.
