...
`firn doctor --json` now reports structured project health details, redacted secret-resolution details, and DuckDB ICU probe details for the scoped CLI doctor surface.
...
Reviewed the scoped diff in `crates/firn-cli/src/commands.rs`, `crates/firn-cli/src/tests.rs`, `crates/firn-cli/tests/doctor_env.rs`, `.10x/specs/project-cli-observability-security.md`, and `.10x/tickets/done/2026-07-06-doctor-secrets-duckdb-icu-health.md`.

Commands and results:

- `cargo fmt --all -- --check`: passed.
- `git diff --check -- . ':(exclude).gitignore'`: passed.
- `cargo test -p firn-cli --locked --no-fail-fast`: passed, including 35 unit tests and the `doctor_env` integration test.
- `cargo clippy -p firn-cli --all-targets --locked -- -D warnings`: passed during worker and parent loops.
...
- `cargo llvm-cov --workspace --locked --summary-only`: passed, 186 tests under instrumentation; total line coverage 78.90%, `firn-cli/src/commands.rs` line coverage 59.69%.
- `cargo machete`: passed, no unused dependencies reported.
- `cargo mutants -p firn-cli --file crates/firn-cli/src/commands.rs --in-diff /tmp/firn-doctor-secrets-icu-commands.diff --test-package firn-cli --jobs 4 --timeout 120 --output target/quality/reports/mutants-doctor-secrets-icu -- --locked`: passed, 6 mutants tested, 4 caught, 2 unviable, 0 missed, 0 timed out.
- `semgrep scan --config p/rust --error --no-git-ignore --json --output target/quality/reports/semgrep-rust-doctor-secrets-icu.json --exclude target --exclude reports --exclude .git .`: passed, 0 results. An explicit follow-up scan of `crates/firn-cli/tests/doctor_env.rs` passed with 0 results after replacing `std::env::temp_dir()` with a workspace-local `target/quality/test-projects` fixture root.
