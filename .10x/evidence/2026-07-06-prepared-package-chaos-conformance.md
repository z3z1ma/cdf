...
Created: 2026-07-06
Updated: 2026-07-06
Relates-To: .10x/tickets/done/2026-07-06-prepared-package-chaos-conformance.md, .10x/tickets/2026-07-05-conformance-chaos-golden.md, .10x/specs/conformance-governance-roadmap.md, .10x/specs/package-lifecycle-determinism.md, .10x/specs/checkpoint-state-firn-line.md, .10x/specs/destination-receipts-guarantees.md

# Prepared-package chaos conformance evidence

## What was observed

The prepared-package DuckDB/SQLite chaos conformance slice is implemented in `crates/firn-conformance/src/package_replay/` and exported from a thin `crates/firn-conformance/src/lib.rs` module root. The harness builds deterministic Arrow IPC package fixtures with caller-supplied `StateDelta`, target, disposition, schema hash, and merge keys; drives the public `firn-project` prepared DuckDB replay and recovery APIs; and asserts durable package receipts, DuckDB `_firn_loads` and `_firn_state` mirror rows, checkpoint heads, duplicate/no-op replay identity, and recovery from a durable receipt without a second destination write.
...
- `cargo test -p firn-conformance --locked --no-fail-fast`: passed, 23 tests.
- `cargo clippy -p firn-conformance --all-targets --locked -- -D warnings`: passed.
- `cargo test -p firn-project --locked --no-fail-fast`: passed, 24 tests.
- `cargo test -p firn-dest-duckdb -p firn-state-sqlite -p firn-package --locked --no-fail-fast`: passed.
...
- `cargo llvm-cov --workspace --all-features --locked --summary-only`: passed. Total coverage was 78.22% regions and 80.94% lines. `crates/firn-conformance/src/package_replay/mod.rs` reported 96.44% regions and 98.32% lines. JSON and LCOV reports were written to `reports/ai-quality/llvm-cov.json` and `reports/ai-quality/lcov.info`.
...
- `cargo +nightly udeps -p firn-conformance --all-targets --locked` with isolated target `target/quality/udeps-prepared-package-target`: passed with "All deps seem to have been used."
- `cargo semver-checks --workspace --baseline-rev HEAD`: passed for all crates with no semver update required.
- Direct owned-source unsafe scan with `rg -n "\bunsafe\b|unsafe\s+impl|unsafe\s+trait|extern\s+\"C\"|from_raw|into_raw|transmute|MaybeUninit|NonNull|UnsafeCell|raw pointer" crates tools python`: found only `crates/firn-http/src/retry.rs` string text and no owned unsafe Rust, FFI, or raw-pointer surface in this change.
- `CARGO_TARGET_DIR=target/quality/careful-prepared-package-target cargo +nightly careful test -p firn-conformance --all-features --locked`: passed, 23 tests.

Mutation testing:

- `cargo mutants --package firn-conformance --file crates/firn-conformance/src/package_replay/mod.rs --test-package firn-conformance --output target/quality/mutants-prepared-package --no-shuffle --jobs 4 --timeout 120 -- --locked`: passed.
...
- `cargo kani -p firn-conformance` is structurally blocked by Kani 0.67.0 compiling `rusqlite 0.40.1`, which uses `cfg_select!` requiring an unstable library feature for that verifier toolchain. This does not indicate a failing conformance test.
- `cargo geiger --all-features` from `crates/firn-conformance` failed after build due third-party dependency parsing and unscanned-file warnings, including `signal-hook-registry-1.4.8/src/lib.rs`. A follow-up `cargo geiger --forbid-only --all-features --locked` attempt repeated the same dependency parser warning for more than a minute and was interrupted with exit code 130. The owned-source unsafe scan above is the recorded project-owned unsafe signal for this slice.
