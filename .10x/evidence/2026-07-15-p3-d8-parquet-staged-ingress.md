...
The selected `arrow_ipc_to_parquet@4` physical plan records its exact two-writer, row/byte batch, deterministic grouping, and content-key derivation policies in attempt metadata before staging mutation. A runtime-owned `StagingLease` renews independently of segment progress and fences every external mutation; Parquet owns no heartbeat, process-local liveness registry, modification-time retention rule, or cleanup eligibility decision. Each completed group is create-or-verify published under its immutable SHA-256 key and its bounded attempt object is removed immediately, so final binding publishes only the manifest/receipt rather than copying the run's accumulated output. Failed multipart uploads are explicitly aborted, expired attempt cleanup requires an exact renewable proof from the generic lease authority, and shared immutable objects are outside attempt cleanup pending the generic reachability authority owned by `.10x/tickets/2026-07-15-p3-d9-content-reachability-authority.md`.
