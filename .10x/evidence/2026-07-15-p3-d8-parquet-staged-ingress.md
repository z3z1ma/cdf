...
3. Ran `/usr/bin/time -lp .../cdf run fineweb.documents --jobs 4 --progress never --color never`, then queried typed phase events from the fresh SQLite ledger, counted/summed destination Parquet objects, and searched the staging hierarchy for files. Run: `run-cf7f7370001f592ec03df926a7b60341`; package hash: `sha256:ee79b5e0857c913dd0530985bbba7dd216215e74ac180b9c105b3f4a79902c12`.
4. Reused the already-recorded two-worker `ArrowParquetRewrite` reference because the v4 repair does not change the source data or destination writer settings that define that comparator.

| `@4` closure metric | Observation |
|---|---:|
| wall / user / system | 19.15 / 49.78 / 33.12 s |
| peak RSS | 1,231,093,760 B |
| package execution | 18.875 s |
| destination ingress, overlapped | 18.410 s |
| final binding + receipt | 0.073 s |
| destination objects / segment acks | 58 / 460 |
| destination output / complete wall | 715.5 MiB/s |
| two-writer same-FineWeb reference | 958.4 MiB/s |
| complete CDF command / favorable same-data reference | 0.747x |
| attempt staging files after success | 0 |

Historical `@3` closure procedure:
...
The selected physical plan records its exact two-writer, row/byte batch, deterministic grouping, content-key derivation, and upload policies in attempt metadata before staging mutation. A runtime-owned `StagingLease` renews independently of segment progress and fences every external mutation; Parquet owns no heartbeat, process-local liveness registry, modification-time retention rule, or cleanup eligibility decision. Each completed group is atomically create-or-verify published under its immutable SHA-256 key, so final binding publishes only the manifest/receipt rather than copying the run's accumulated output. Generic process-local multipart handles were deleted because they cannot recover a provider upload id after process loss or an ambiguous initiation response; remote/custom stores now use one ledger-accounted atomic bounded-object PUT, and a future multipart capability must expose persistent recovery identity. Expired attempt cleanup requires an exact renewable proof from the generic lease authority. Shared immutable objects are outside attempt cleanup pending the generic reachability authority owned by `.10x/tickets/2026-07-15-p3-d9-content-reachability-authority.md`.
