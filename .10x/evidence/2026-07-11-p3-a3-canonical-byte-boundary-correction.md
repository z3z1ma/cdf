...
This supports plan-effective row/byte segmentation for primitive and variable-width flat arrays and fixes position authority at flush boundaries. It challenges the earlier progress claim that the serialized byte ceiling was already live.

## Limits

A3 remains open. Nested/list, dictionary, union, run-end, and view-array logical-byte rechunking require explicit conformance before the estimator can claim the full Arrow vocabulary. A5 still owns transferring already-accounted envelopes into assembler retention so physical shared-buffer ownership is not reconstructed from per-batch estimates.
