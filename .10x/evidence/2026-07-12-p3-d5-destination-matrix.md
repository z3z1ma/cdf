...
The registered schema-ineligible fixtures were rejected during preflight: DuckDB `decimal256-v1`, PostgreSQL `time32-microsecond-invalid-v1` (an invalid Arrow Time32/unit pairing), and Parquet `month-day-nano-interval-v1`. No compatibility/scalar path was selected. These concrete names replace the former ambiguous `unsupported-arrow-v1` label; ordinary PostgreSQL nested, dictionary, interval, duration, union, and run-end-encoded values are eligible through the allowance-gated JSONB mapping.
...
2026-07-21 descriptor-authority refresh: the destination-fidelity expansion changed schema preflight semantics without changing any selected execution path, tuning range, writer topology, or measured implementation. Destination observations now bind both authorities separately: a canonical execution-descriptor SHA-256 over every execution-affecting descriptor field, and the exact schema-preflight version. The retained EC2 timing samples remain labeled with their original measured evidence versions rather than being relabeled as new measurements. Current preflight conformance revalidated `tlc-v1`, `tpch-orders-v1`, `wide-entropy-v1`, `decimal256-v1`, `time32-microsecond-invalid-v1`, and `month-day-nano-interval-v1` against the `@2` mappings.
