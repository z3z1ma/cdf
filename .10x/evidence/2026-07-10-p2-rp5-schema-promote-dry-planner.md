...
The positive lossy test exercises the typed path helper with an explicitly constructed `TypePolicy`; it is not a full-planner positive test. The current Tier-0 compiled resource authority derives both lossy and parse coercion allowances as false. Making `LossyAllowed` reachable through the full planner remains blocked on `.10x/tickets/2026-07-09-p2-ws-g2-type-mismatch-diagnostics.md` and must satisfy `.10x/knowledge/type-policy-authority.md`; RP5 remains fail-closed and does not treat runtime injection as authority.
