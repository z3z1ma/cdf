...
`firn-engine` now preserves the existing `execute_to_package` API and exposes `execute_to_package_with_run_id`, which requires a caller-supplied `RunId`. The traced path emits `firn_engine.package_execution` spans with exact `run_id`, `resource_id`, and `package_id` fields, plus `firn_engine.partition_execution` spans with exact `run_id`, `resource_id`, `package_id`, and `partition_id` fields.

The tracing tests capture spans through a local subscriber and compare exact field maps. A traced/untraced package execution comparison proves tracing does not change manifest identity, package hash, or signature. Mutation testing initially found unrelated surviving engine execution assertions; final test hardening killed all viable mutants in `crates/firn-engine/src/execution.rs`.
...
- `cargo test -p firn-engine --locked --no-fail-fast`: passed; 10 unit tests and 0 doctests.
- `cargo clippy -p firn-engine --all-targets --locked -- -D warnings`: passed.
- `cargo mutants --file crates/firn-engine/src/execution.rs --all-features --cargo-arg --locked --jobs 2 --test-tool cargo --output reports/ai-quality/mutants-engine-tracing -- -p firn-engine`: passed; 29 mutants tested, 18 caught, 11 unviable, 0 missed.
- `MIRIFLAGS=-Zmiri-disable-isolation CARGO_TARGET_DIR=target/quality/miri-engine-tracing-target cargo +nightly miri test -p firn-engine traced_execution_emits_run_resource_package_and_partition_spans --locked`: passed; 1 targeted tracing test.
- `CARGO_TARGET_DIR=target/quality/careful-engine-tracing-target cargo +nightly careful test -p firn-engine --all-features --locked`: passed; 10 unit tests and 0 doctests. The macOS MainThreadChecker dylib warning is a local optional-checker availability warning, not a test failure.
...
- `cargo llvm-cov --workspace --all-features --locked --summary-only`: passed on the final tree; total line coverage 81.24%, `firn-engine/src/execution.rs` line coverage 90.59%.

Dependency, API, security, and supply-chain checks:

- `cargo tree -p firn-engine --locked`: confirmed `firn-engine` has a direct `tracing v0.1.44` dependency.
- `cargo machete`: passed; no unused dependency candidates.
- `CARGO_TARGET_DIR=target/quality/udeps-engine-tracing-target cargo +nightly udeps -p firn-engine --all-targets --locked`: passed; all deps used.
...
- Direct final source scan `rg -n "\bunsafe\b|unsafe\s+impl|unsafe\s+trait|extern\s+\"C\"|from_raw|into_raw|transmute|MaybeUninit|NonNull|UnsafeCell" crates/firn-engine/src Cargo.lock crates/firn-engine/Cargo.toml`: no engine source or manifest unsafe markers; only existing transitive crate name `unsafe-libyaml` in `Cargo.lock`.
- `CARGO_TARGET_DIR=target/quality/geiger-engine-tracing-target cargo geiger --manifest-path "$PWD/crates/firn-engine/Cargo.toml" --all-targets --all-features --include-tests --locked --output-format Json > target/quality/reports/geiger-engine-tracing.json 2> target/quality/reports/geiger-engine-tracing.stderr`: passed. `firn-engine` used unsafe counts are 0 functions, 0 expressions, 0 unsafe impls, 0 unsafe traits, and 0 unsafe methods.
- `rust-code-analysis-cli -m -p crates/firn-engine/src -O json --pr` streamed 6 JSON metric documents, wrapped with `jq -s` into `reports/ai-quality/rust-code-analysis-engine-tracing-array.json`.
- `jscpd crates/firn-engine/src --format rust --reporters console,json --output reports/ai-quality/jscpd-engine-tracing --min-lines 12 --min-tokens 80 --threshold 0 --exit-code 0 --no-colors`: passed; 0 clones, 0 duplicated lines, 0.00%.
