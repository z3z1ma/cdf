...
`cdf-engine` now contains an MVP planner/execution surface for mock resources:

- Tier A resources use engine-side residual filtering, projection, and limit handling and can write package-ready Arrow IPC segments through `cdf-package`.
...
- Execution reuses `cdf-contract::ValidationProgram` and `cdf-package::PackageBuilder`; no cross-crate API additions were made.
...
It also supports the architecture-layering constraint that `cdf-engine` composes kernel, contract, and package semantics rather than redefining them.

## Limits

The MVP does not implement concrete HTTP, Python, subprocess, destination drivers, or external resource implementations.

DataFusion 54 currently depends on Arrow 58.3 while the lower CDF crates use Arrow 59. The MVP keeps DataFusion types isolated to planner-facing fidelity mapping and operator-chain identity, and uses Arrow 59 batches for CDF execution/package output so lower-layer public APIs remain unchanged.
