...
- `cargo test -p firn-conformance --locked --no-fail-fast`: passed, 12 tests.
- `cargo test -p firn-dest-duckdb --locked --no-fail-fast`: passed, 9 tests.
- `cargo test -p firn-dest-parquet --locked --no-fail-fast`: passed, 15 tests.
- `cargo clippy -p firn-conformance --all-targets --locked -- -D warnings`: passed after repairing a duplicate-branch lint.
- `cargo clippy -p firn-dest-duckdb --all-targets --locked -- -D warnings`: passed.
- `cargo clippy -p firn-dest-parquet --all-targets --locked -- -D warnings`: passed.
- `cargo mutants -p firn-conformance --file crates/firn-conformance/src/destination/mod.rs --jobs 4 -o target/quality -- --locked -p firn-conformance -p firn-dest-duckdb -p firn-dest-parquet --no-fail-fast`: passed with 13 mutants tested, 7 caught, 6 unviable, 0 missed. Raw outcome file: `target/quality/mutants.out/outcomes.json`.
...
- `rg -n "\bunsafe\b|unsafe\s+impl|unsafe\s+trait|extern\s+\"C\"|from_raw|into_raw|transmute|MaybeUninit|Unpin|Send|Sync" crates`: found no unsafe blocks or FFI in Firn source; matches were trait bounds/type aliases or string text.
...
- `cargo geiger --manifest-path crates/firn-cli/Cargo.toml --all-features --all-dependencies --include-tests --locked --output-format Ratio`: completed analysis and wrote `target/quality/geiger-ratio.txt`, then exited nonzero because dependency scanner warnings were treated as errors. The summary showed Firn crates at 100% safe ratios; the dependency-wide ratio was 94.07% functions, 94.12% expressions, 96.41% items, 94.95% impls, 96.95% lines. A narrower conformance retry was stopped after it again behaved as a dependency-wide warning-heavy scan without producing a useful report.
...
`cargo deny check` and `cargo vet` remain blocked by unratified supply-chain policy, not by this destination-conformance implementation. `cargo geiger` was limited by scanner warnings from registry/generated dependency files; direct source search found no Firn unsafe blocks or FFI. Miri, cargo-careful, fuzzing, Kani, benchmarks, and bloat checks were not run because this slice added no unsafe code, proof harnesses, fuzz targets, or performance-sensitive runtime paths.
