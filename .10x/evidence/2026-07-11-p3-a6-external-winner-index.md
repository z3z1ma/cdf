...
- `cargo test -p cdf-engine append_exact_row_dedup_compiles_and_drops_only_complete_duplicates -- --nocapture` — passed both the simple reference evaluator and forced production spill path, with byte-identical manifest identity/package hash and a balanced spill budget after cleanup.
