...
The explicit local-file `cdf run` slice now executes one declarative local file resource through `cdf-engine`, writes a package under the selected environment package root, commits the package to a local DuckDB destination, verifies the DuckDB receipt, and commits a SQLite checkpoint only after receipt verification.

The implementation is scoped to:

- `crates/cdf-engine`: semver-additive segment source-position reporting for package execution.
- `crates/cdf-project`: reusable local file to DuckDB/SQLite run orchestration plus receipt/checkpoint recovery tests.
- `crates/cdf-cli`: explicit `run --resource --pipeline --target --package-id --checkpoint-id` wiring, JSON/human output, and no-write negative tests.

## Behavioral evidence

- `cargo test -p cdf-engine -p cdf-project -p cdf-cli --locked --no-fail-fast` passed after the final mutation-hardening patch. This covered the focused engine segment-position test, project live-run and failure-window tests, and CLI success/negative tests.
- `cargo nextest run -p cdf-engine -p cdf-project -p cdf-cli --locked` passed with 108 focused tests.
...
- `semgrep scan --config p/rust --error --json --output target/quality/semgrep-local-file-run-rust-final.json crates/cdf-engine crates/cdf-project crates/cdf-cli` passed with 0 findings.
- `semgrep scan --config p/security-audit --error --json --output target/quality/semgrep-local-file-run-security-final.json crates/cdf-engine crates/cdf-project crates/cdf-cli` passed with 0 findings.
...
- `jscpd --min-lines 8 --min-tokens 80 --reporters console crates/cdf-project/src/runtime.rs crates/cdf-cli/src/commands.rs` passed with 0 clones.
- `rust-code-analysis-cli -m -O json -p crates/cdf-project/src/runtime.rs` completed. Highest reported function cyclomatic values in the touched runtime file were 14 for `replay_prepared_duckdb_package`, 12 for `run_local_file_to_duckdb_checkpoint`, and 10 for `state_delta_from_run`; these are orchestration functions with focused negative tests and mutation coverage.
- `tokei crates/cdf-engine/src crates/cdf-project/src crates/cdf-cli/src --output json` completed. It reported 11,033 Rust code lines across those three source trees.
- `scc --format json crates/cdf-engine/src crates/cdf-project/src crates/cdf-cli/src` completed. It reported 8,999 code lines across 25 Rust files.
- `git diff --check -- . ':(exclude).gitignore'` passed before record closure.

Mutation testing:

- Initial bounded runtime mutation run:
  - Command: `cargo mutants --package cdf-project --file crates/cdf-project/src/runtime.rs --cargo-arg=--locked --jobs 2 --test-tool cargo --timeout 900 --output target/quality/mutants-local-file-run-runtime -- -p cdf-project -p cdf-cli -- --nocapture`
