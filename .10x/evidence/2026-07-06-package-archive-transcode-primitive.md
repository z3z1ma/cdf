...
`firn-package` now exposes `archive_package_to_parquet` and a serializable in-memory archive report. The primitive opens and verifies a package before reading IPC segments, transcodes each segment to Parquet bytes through the existing DuckDB-backed writer path, and records package hash, fidelity statement, source IPC path/hash/bytes/rows, Parquet bytes/hash/bytes/rows, and segment id.

The DuckDB Parquet writer implementation moved from `firn-dest-parquet` into focused `firn-package` module files. `firn-dest-parquet` delegates to the shared writer, so destination commit, object-store, manifest, and receipt semantics remain in the destination crate. `crates/firn-package/src/lib.rs` remains a thin module/export root rather than a monolithic implementation file.
...
- Focused worker verification passed before parent review: `cargo fmt --all -- --check`; `cargo test -p firn-package --locked --no-fail-fast`; `cargo test -p firn-dest-parquet --locked --no-fail-fast`; `cargo clippy -p firn-package -p firn-dest-parquet --all-targets --locked -- -D warnings`; `cargo deny check advisories`; `git diff --check -- . ':(exclude).gitignore'`; and `rg -n '^name = "(parquet|paste)"' Cargo.lock crates/firn-package/Cargo.toml crates/firn-dest-parquet/Cargo.toml` with no matches.
- Parent review ran `cargo mutants --package firn-package --file crates/firn-package/src/archive.rs --file crates/firn-package/src/parquet.rs --locked --output target/quality/mutants-archive-primitive`. First run: 27 mutants, 1 missed, 20 caught, 6 unviable. The missed mutant was `replace validate_field_names -> Result<()> with Ok(())`.
- After adding duplicate-column prevalidation coverage, final `cargo mutants --package firn-package --file crates/firn-package/src/archive.rs --file crates/firn-package/src/parquet.rs --locked --output target/quality/mutants-archive-primitive` passed: 27 mutants, 0 missed, 21 caught, 6 unviable.
...
- Final coverage gate passed: `cargo llvm-cov --workspace --locked --summary-only`. Total coverage was 76.64% regions and 79.34% lines. New archive files measured at `firn-package/src/archive.rs` 80.39% line coverage and `firn-package/src/parquet.rs` 58.01% line coverage.
- Final dependency and supply-chain gates passed:
  ...
  - `rg -n '^name = "(parquet|paste)"|parquet =|paste =' Cargo.lock crates/firn-package/Cargo.toml crates/firn-dest-parquet/Cargo.toml`: no matches.
- Final security/static gates passed:
  - `semgrep scan --config auto --error --json --output target/quality/semgrep-archive-primitive-final.json crates/firn-package crates/firn-dest-parquet`: 0 findings.
