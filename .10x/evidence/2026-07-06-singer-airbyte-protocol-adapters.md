...
`firn-subprocess` now exposes focused `protocol`, `singer`, and `airbyte` modules behind a thin crate root. The adapters parse newline-delimited JSON protocol messages, retain raw parsed envelopes for forward-compatible unknown fields, convert records into per-stream `FormatRead` values with stream identity preserved in scope and batch IDs, and map Singer/Airbyte opaque state into `SourcePosition::ForeignState`.
...
The existing dirty `.gitignore` remained unrelated and untouched. `Cargo.lock` changed only to record the new direct `firn-subprocess` JSON/hash dependencies required by `crates/firn-subprocess/Cargo.toml`.

## Procedure

- Re-read `firn-the-book-of-the-system.md` sections for Tier 4 subprocess adapters, Singer/Airbyte mapping, `ForeignState`, bridge discipline, and Airbyte destination exclusion. Result: this ticket remained scoped to parser/batch/state bridging; package archive and Airbyte destinations stayed excluded.
- Ran `cargo test -p firn-subprocess --no-fail-fast` once after adding direct dependencies so Cargo could refresh lockfile metadata required by later `--locked` verification. Result before parent hardening: passed, 10 unit tests and 0 doc tests.
- Ran focused final adapter checks after parent review and mutation hardening:
  - `cargo fmt --all -- --check`: passed.
  - `cargo test -p firn-subprocess --locked --no-fail-fast`: passed, 12 unit tests and 0 doc tests.
  - `cargo clippy -p firn-subprocess --all-targets --locked -- -D warnings`: passed.
  - `cargo +nightly careful test -p firn-subprocess --locked`: passed, 12 unit tests and 0 doc tests. Installed nightly/rust-src as required by the tool.
  - `cargo mutants --package firn-subprocess --no-shuffle --jobs 4 --timeout 120 --output target/quality/reports/mutants-singer-airbyte`: final result 109 mutants tested, 82 caught, 27 unviable, 0 missed.
- Ran workspace compile/lint/test gates:
  ...
  - `cargo test --workspace --all-targets --locked --no-fail-fast`: passed, including 12 `firn-subprocess` tests.
...
- Ran coverage, dependency, maintainability, and size checks:
  ...
  - `cargo bloat --release -p firn-cli --bin firn-cli -n 20`: passed; top entries remain DuckDB symbols, `.text` 28.9 MiB and file size 45.6 MiB.
- Ran security/supply-chain checks:
  ...
  - `CARGO_TARGET_DIR=target/quality/geiger-target cargo geiger --manifest-path /Users/alexanderbut/code_projects/personal/firn/crates/firn-subprocess/Cargo.toml --all-targets --all-features --include-tests --locked`: completed inventory but exited 1 with 5 tool/dependency warnings and dependency unsafe counts. The `firn-subprocess` local package lines showed no local unsafe blocks.
  - `semgrep scan --config p/rust --error crates/firn-subprocess/src/protocol.rs crates/firn-subprocess/src/singer.rs crates/firn-subprocess/src/airbyte.rs crates/firn-subprocess/src/tests.rs`: passed, 0 findings.
  - `semgrep scan --config p/security-audit --error crates/firn-subprocess/src/protocol.rs crates/firn-subprocess/src/singer.rs crates/firn-subprocess/src/airbyte.rs crates/firn-subprocess/src/tests.rs`: passed, 0 findings.
...
The implementation is scoped to `firn-subprocess` parser/batch/state bridging. It does not exercise full connector lifecycle execution, Airbyte destinations, Parquet archive/transcode behavior, state migration UI, or package archive CLI behavior, all of which are explicit exclusions from the ticket.
