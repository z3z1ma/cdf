...
Created: 2026-07-06
Updated: 2026-07-06
Relates-To: .10x/tickets/2026-07-05-cli-surface.md, .10x/specs/project-cli-observability-security.md, .10x/specs/resource-authoring-planning-batches.md, .10x/specs/package-lifecycle-determinism.md, .10x/specs/checkpoint-state-firn-line.md

# CLI surface evidence

## What was observed

`crates/firn-cli` now has a real command surface split across focused modules:
...
- `src/context.rs` loads `firn.toml`, compiles declarative resources through `firn-project`, parses `firn.lock` when present, resolves package roots, opens SQLite checkpoint stores through `firn-state-sqlite`, and exposes destination capability reports through current destination crates.
...
- `init`: no project scaffold/write API in `firn-project`.
...
```text
cargo check -p firn-cli
cargo test -p firn-cli --locked --no-fail-fast
cargo fmt --all -- --check
cargo clippy -p firn-cli --all-targets --locked -- -D warnings
cargo check --workspace --all-targets --locked
cargo check -p firn-cli --all-targets --locked
```

The first `cargo test -p firn-cli --locked --no-fail-fast` run initially failed two new tests after formatting:
...
Both failures were repaired in `crates/firn-cli`.
...
All commands passed. Semgrep initially flagged `std::env::args_os` in `src/main.rs` and `std::env::temp_dir` in `src/tests.rs`; parent changed test directories to a source-local `target/firn-cli-tests/<unique>` root and replaced `args_os` with `args().map(OsString::from)` plus a narrow `nosemgrep` comment because command-line argv dispatch is not a security boundary in this CLI.
