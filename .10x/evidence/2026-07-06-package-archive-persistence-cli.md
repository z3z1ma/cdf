...
`cdf-package` now persists the package Parquet archive contract from `.10x/specs/package-lifecycle-determinism.md`:
...
- `cdf package archive <DIR> [--format parquet] [--force] [--json]` parses through the CLI, appears in help, rejects unsupported formats, prints human output, and emits structured JSON.
...
- `cargo check -p cdf-package -p cdf-cli --all-targets --locked` passed.
- `cargo test -p cdf-package --locked --no-fail-fast` passed with 22 package tests.
- `cargo test -p cdf-cli package_archive --locked --no-fail-fast` passed.
- `cargo test -p cdf-cli help_lists_required_command_surface --locked --no-fail-fast` passed.
- `cargo test -p cdf-cli package_verify_uses_lower_package_reader --locked --no-fail-fast` passed.
- `cargo clippy -p cdf-package -p cdf-cli --all-targets --locked -- -D warnings` passed.
- `cargo nextest run -p cdf-package -p cdf-cli --locked` passed with 69 tests run and 69 passed.
...
- `semgrep scan --config p/rust --error --json --output target/quality/semgrep-package-archive-rust.json crates/cdf-package crates/cdf-cli` passed with 0 findings.
- `rg -n '^name = "(parquet|paste)"' Cargo.lock crates/cdf-package/Cargo.toml crates/cdf-cli/Cargo.toml crates/cdf-dest-parquet/Cargo.toml` found no direct lockfile/package entries for `parquet` or `paste`.
- `rg -n '\bunsafe\b|extern "|raw pointer|\*const|\*mut|unsafe impl (Send|Sync)' crates/cdf-package crates/cdf-cli` found no unsafe Rust in the changed crates.
- `cargo +nightly udeps -p cdf-package -p cdf-cli --all-targets --locked` passed.
...
- Initial `cargo mutants --package cdf-package --file crates/cdf-package/src/archive.rs --jobs 4 --timeout 300 --output target/quality/mutants-package-archive-persistence --test-tool cargo --cargo-arg --locked` found 19 missed mutants.
