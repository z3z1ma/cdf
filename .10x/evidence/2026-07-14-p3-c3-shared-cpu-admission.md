...
- `fixed_pool_teardown_is_safe_when_worker_releases_the_last_owner` constructs a one-worker pool, captures the only remaining pool owner inside that worker, and requires the completion after the worker drops it. It passed 20 consecutive runs; the complete 22-test host set and strict engine all-target Clippy also passed. Teardown detaches only the current handle and still joins every other worker after setting the shared shutdown state.
