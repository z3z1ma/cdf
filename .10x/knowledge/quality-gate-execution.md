...
This local `cargo-mutants` version does not accept Cargo flags such as `--locked` directly. Pass them through `--cargo-arg`, for example `cargo mutants --file crates/firn-engine/src/execution.rs --all-features --cargo-arg --locked --jobs 2 --test-tool cargo --output reports/ai-quality/mutants-engine-tracing -- -p firn-engine`.
