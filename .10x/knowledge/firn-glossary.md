...
`firn` is the CLI binary and crate prefix; firn in prose is the project.
...
Firn line: the commit boundary enforced by `CheckpointStore::commit`; a source cursor may advance only after all data represented by the cursor is durably committed and the destination receipt is recorded.
...
Firn: a load package: compacted evidence that can still melt if it never commits.
