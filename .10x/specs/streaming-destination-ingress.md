...
Abort MUST be idempotent. Rollback/redrive destinations MUST delete the exact recorded attempt before reusing its identity; an in-process active-attempt registry MUST prevent one session from deleting a live sibling. Successful binding and explicit abort MUST remove the exact attempt immediately. Persistent object-store attempts MUST write a heartbeat at session start and at least once per minute while work continues. An attempt whose newest object or heartbeat is older than seven days MAY be collected; a process-local active attempt MUST never be collected. The collector MUST delete only the exact attempt prefix, MUST NOT infer committed state from attempt identity, and MUST report storage errors rather than claim cleanup. Recovery MAY reattach only after destination-reported staging identities exactly match the run ledger and final package where available.
