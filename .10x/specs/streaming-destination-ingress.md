...
Every externally staged namespace and cleanup candidate MUST record the lease identity and fencing token that created it. A restarted attempt MUST use a fencing-token-distinct namespace, so cleanup of an older generation cannot delete newer work. Successful binding and explicit abort MUST remove the exact attempt generation immediately. Process-loss cleanup MUST require an authoritative `prove_expired` result for the recorded generation; object age, modification time, process-local ownership, or completed-segment activity is never sufficient evidence. The collector MUST delete only the exact proven-expired generation, MUST NOT infer committed state from attempt identity, and MUST report storage errors rather than claim cleanup. Recovery MAY reattach only while holding the matching live lease and after destination-reported staging identities exactly match the run ledger and final package where available.
