...
Abort MUST be idempotent. Rollback/redrive destinations MUST delete the exact recorded attempt before reusing its identity; an in-process active-attempt registry MUST prevent one session from deleting a live sibling. Staging garbage collection MUST be bounded by an explicit lifecycle/retention policy and MUST never infer committed state from attempt identity. Recovery MAY reattach only after destination-reported staging identities exactly match the run ledger and final package where available.
