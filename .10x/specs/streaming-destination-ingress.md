...
Abort MUST be idempotent. Every externally durable staging attempt MUST execute under one runtime-owned `StagingLease`; destination adapters MUST NOT create their own heartbeat, liveness thread, or retention eligibility policy. The lease identity binds destination, target, and load attempt; acquisition assigns a monotonically increasing fencing token. The runtime or artifact/storage authority MUST acquire and renew the lease independently of segment completion, MUST retain it through final binding or abort, and MUST release it only after all adapter work is structurally joined.

Every externally staged namespace and cleanup candidate MUST record the lease identity and fencing token that created it. A restarted attempt MUST use a fencing-token-distinct namespace, so cleanup of an older generation cannot delete newer work. Successful binding and explicit abort MUST remove the exact attempt generation immediately. Process-loss cleanup MUST atomically advance the fencing generation into a renewable cleanup lease as part of the authoritative `prove_expired` operation; a passive proof followed by deletion is forbidden because another writer could acquire between those steps. Object age, modification time, process-local ownership, or completed-segment activity is never sufficient evidence. The collector MUST hold the cleanup lease until deletion finishes, MUST delete only namespaces exclusively owned by the exact proven-expired generation, and MUST report storage errors rather than claim cleanup. Shared content-addressed objects that a later generation can reuse are not staging-owned and MUST NOT be deleted by this path; their reclamation requires independent reachability authority. Recovery MAY reattach only while holding the matching live lease and after destination-reported staging identities exactly match the run ledger and final package where available.
