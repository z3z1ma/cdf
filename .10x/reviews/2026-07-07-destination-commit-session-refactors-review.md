...
Significant residual design constraint: the kernel `DestinationProtocol::begin(request, plan)` surface does not carry package path, package schema, merge-key, package view, or destination-specific replay context. DuckDB and Parquet bridge this by remembering package context from `plan_package_commit`; Postgres bridges it with `with_commit_request`. This is acceptable for the current refactor because the tickets required compatibility with existing package commit paths, but the general orchestrator must make package-aware session input first-class or deliberately refine the kernel API. This is recorded on `.10x/tickets/done/2026-07-07-general-run-orchestrator.md`.
