...
The Singer/Airbyte protocol adapter child covers Singer `SCHEMA`/`RECORD`/`STATE`, Airbyte catalog/state parsing, scoped opaque `ForeignState` mapping, firn-line state hashing, parser tests, package write/replay compatibility, mutation evidence, and quality evidence.
