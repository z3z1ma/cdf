...
Partial implementation for `.10x/tickets/done/2026-07-07-cli-replay-package-spine.md`, covering `cdf replay package <DIR> --to duckdb://path` only.

## Findings

- Significant: At review time, the ticket acceptance criteria were not fully satisfied. Filesystem Parquet replay was still blocked on ratified CLI destination URI spelling, and Postgres replay was still blocked on explicit target, merge-dedup, and existing-table policy CLI input shape. The ticket correctly remained `blocked` at that time.
...
Concerns raised. At review time, the DuckDB slice was acceptable progress to commit, but the ticket could not close until the Parquet and Postgres blockers were ratified and implemented or explicitly descoped.
