...
What protocol behavior is sufficiently concrete to implement the first CDF Singer/Airbyte subprocess adapter slice without inventing semantics or touching the Parquet archive work that remains supply-chain gated?

## Sources and methods

- Inspected `.10x/specs/resource-authoring-planning-batches.md`, `.10x/specs/checkpoint-state-commit-gate.md`, `.10x/specs/package-lifecycle-determinism.md`, and `.10x/tickets/done/2026-07-05-singer-airbyte-and-package-archive.md`.
...
- `STATE` messages require `value`, and the value's semantics are tap-defined. CDF MUST preserve it as opaque foreign state unless a later adapter knows the tap-specific meaning.
...
- State messages include legacy black-box state and newer stream/global state. Airbyte explicitly treats state contents as black-box data owned by the source, so CDF should map them to scoped `ForeignState` positions rather than interpreting the state blob.

## Conclusions

Open a focused child ticket for Singer/Airbyte protocol parsing and `ForeignState` mapping in `cdf-subprocess`. The child can use existing JSON and Arrow/NDJSON machinery, add no Parquet dependency, and preserve the crate-organization convention by using focused modules rather than growing `src/lib.rs`.

Do not include `cdf package archive` in that child. Archive Parquet transcode remains blocked by `.10x/tickets/done/2026-07-06-parquet-format-source-supply-chain.md` or by a later ratified supply-chain policy/alternative writer.
