...
6. Both tested forms dynamically linked `liblz4.so.1` because nanoarrow `0.8.0` resolves LZ4 through the platform CMake/pkg-config package. A portable CDF release must either link a pinned LZ4 archive into the custom DuckDB library or package the matching runtime library. It must not silently acquire a new undeclared host dependency.
...
- The controlled static library currently has a dynamic `liblz4.so.1` dependency. Portable release packaging remains incomplete until LZ4 is statically linked or carried and verified per target.
