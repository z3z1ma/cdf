...
How should Firn satisfy the book/spec requirement that packages contain `state/proposed_delta.json` and `destination/commit_plan.json` when the runtime `StateDelta` and destination commit request include values derived from the package hash itself?

## Sources and methods

- Read `firn-the-book-of-the-system.md` Chapter 11, especially the package layout and hashing sections.
- Read `.10x/specs/package-lifecycle-determinism.md`, `.10x/specs/checkpoint-state-firn-line.md`, and `.10x/specs/destination-receipts-guarantees.md`.
- Inspected `crates/firn-package/src/storage.rs`, `crates/firn-package/src/builder.rs`, `crates/firn-package/src/model.rs`, and package tests.
- Inspected `crates/firn-engine/src/execution.rs` for current engine-produced package artifacts.
- Inspected `crates/firn-project/src/runtime.rs` for runtime `StateDelta` and DuckDB commit construction.
...
This preserves the book's intent that package identity covers the planned state/destination evidence, while avoiding an impossible hash cycle. It also unblocks a future `firn replay package` path that can reconstruct the explicit state delta and commit inputs from package evidence without guessing.
