...
Can Firn replace the DuckDB-backed Parquet workaround with the native arrow-rs/DataFusion Parquet stack by using the latest package versions, or should it temporarily accept `RUSTSEC-2024-0436` because native Parquet is architecturally central?
...
- Current Firn manifests and policy:
  - `crates/firn-engine/Cargo.toml`
  - `crates/firn-package/Cargo.toml`
  - `crates/firn-formats/Cargo.toml`
  - `crates/firn-dest-parquet/Cargo.toml`
  - `deny.toml`
...
- Current Firn intentionally uses `datafusion = { version = "54.0.0", default-features = false }` in `firn-engine`, so the workspace can use the DataFusion planning/execution boundary without activating DataFusion's Parquet feature yet.
- Current Firn Parquet source, destination, and archive paths use DuckDB's bundled Parquet support:
  - `firn-package` depends on `duckdb` with `bundled, parquet`.
  - `firn-formats` depends on `duckdb` with `bundled, parquet` and bridges DuckDB Arrow 58 IPC into Firn Arrow 59.
  - `firn-dest-parquet` depends on `duckdb` with `bundled, parquet`.
...
There is no architectural reason Firn cannot use native arrow-rs/DataFusion Parquet. In fact, native Parquet aligns better with the book because DataFusion and Arrow are load-bearing infrastructure, while the DuckDB Parquet path is an FFI workaround with Arrow-major bridging.

The reason Firn has not simply switched is narrower: latest upstream versions do not remove `paste`, and the current active supply-chain policy deliberately allows no advisory ignores. The workaround was a policy-preserving implementation path, not a technical preference.
...
- Owner: Firn dependency policy / parent implementation plan.
- Scope: `paste` via `parquet` / DataFusion Parquet only; no broad unmaintained-advisory ignore.
- Expiry/revisit: every Arrow/DataFusion upgrade and no later than the next Firn minor dependency-pin review.
