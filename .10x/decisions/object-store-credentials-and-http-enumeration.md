...
- HTTP multi-file resources MUST use finite, deterministic template expansion. The supported initial grammar is an inclusive numeric range `{start..end}` with width preservation (for example `{01..12}`). Unbounded `*`, `?`, and `**` over HTTP remain contract errors because HTTP has no portable LIST operation.
