...
- Acceptance mapping: exact pin/build configuration is recorded in the Journal; extension load/version, generated LZ4, and current-segment compatibility are proved by the controlled-host probes; the raw JSON records every full-year sample, row count, throughput, RSS, cgroup peak/events, and measurement biases. D15's canonical ordinal and the full-CDF destination-owned product path are implemented; release-artifact static embedding and its zero-configuration EC2 run remain open.
