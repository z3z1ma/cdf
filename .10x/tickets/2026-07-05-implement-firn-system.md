...
- 2026-07-06: Opened observability child `.10x/tickets/2026-07-06-engine-execution-tracing-spans.md` after read-only exploration showed `inspect run` would invent unratified run-ledger semantics. The child is scoped to an additive explicit-run-id tracing entry point in `firn-engine`.
