...
- 2026-07-18: Refreshed EC2 to the corrected provider and reran the doctor probe. The product now reads the scoped cgroup path (`/sys/fs/cgroup/user.slice/user-1000.slice/session-781.scope`) and records `memory.current`, `memory.peak`, and `memory.events` without read errors. `memory.max` is unbounded (`max`) for the current session, so the host remains valid for throughput benchmarking but is not yet an enforced constant-memory proof host.
- 2026-07-18: Probed transient cgroup enforcement on the same EC2 host. Direct child-cgroup creation under the SSH session is denied to `ec2-user`, but `systemd-run --user -p MemoryMax=...` works and creates bounded cgroup v2 service scopes without sudo. Ran `cdf doctor --json` inside a `MemoryMax=6G` transient user service; the runtime memory check passed with `enforcement = linux_cgroup_v2`, bounded `memory.max = 6442450944`, and separate cgroup current/peak/events plus managed-pool snapshot.
...
- 2026-07-18: `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-current-cgroup-runtime-memory-doctor.json` records the corrected EC2 `cdf doctor --json` memory-authority observation at commit `f16c2745627db6a0809853ce46cf2a4db373110e`. The cgroup provider root is `/sys/fs/cgroup/user.slice/user-1000.slice/session-781.scope`, `current_bytes = 13058048`, `peak_bytes = 16097280`, `events` includes zero OOM counters, and `read_errors` is empty. `max_bytes = null` because `memory.max` is unbounded, so enforcement remains `unavailable`. Limit: unrelated doctor checks still made the overall command exit nonzero; this evidence concerns the parsed `runtime_memory_budget` check.
- 2026-07-18: `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-current-cgroup-revision.env` and `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-current-cgroup-build.env` record the synced/build revision for the corrected current-cgroup EC2 observation. Limit: build markers prove source and binary identity, not memory enforcement.
- 2026-07-18: `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-systemd-cgroup-runtime-memory-doctor.json` records `target/release/cdf --project ... doctor --json` executed inside `systemd-run --user --wait --collect -p MemoryMax=6G`. The `runtime_memory_budget` check passed with cgroup root `/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/run-u0.service`, `max_bytes = 6442450944`, `current_bytes = 12406784`, `peak_bytes = 15388672`, zero OOM counters, and no cgroup read errors. Limit: overall doctor status was `1` due unrelated workspace health checks, recorded separately in `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-systemd-cgroup-doctor.status`; this observation is diagnostic, not a full run/RSS stress proof.
- 2026-07-18: `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-systemd-cgroup-revision.env` and `.10x/evidence/.storage/2026-07-18-p3-f1-ec2-systemd-cgroup-build.env` record the synced/build revision for the systemd-enforced cgroup observation. Limit: build markers prove source and binary identity, not process-tree RSS.
...
- 2026-07-18 self-review: The systemd probe confirms the same EC2 host can become an enforced-memory host for command/run scopes without root-owned manual cgroup directories. This should be productized in the benchmark helper before stress evidence so F1 observations are repeatable and not one-off shell incantations.
