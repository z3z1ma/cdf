...
- 2026-07-19: Extended the real local v2 fixture with one global equality delete and one referenced position delete. Both canonical task construction and the official aligned reader now prove the same semantics end to end: the position delete applies only to its named old file, the equality delete applies to both files by sequence, six of eight rows survive, schema evolution/default filling remains correct, and jobs 1/16 produce identical scan/task identities.
...
- `CARGO_BUILD_JOBS=12 cargo test -p cdf-source-iceberg -p cdf-task-store --lib --locked -j 12`: 26 Iceberg tests and 4 task-store tests passed; the slow million-task task-store law remains explicitly ignored in the fast tier. The local Iceberg fixture physically applies both delete forms and proves six exact surviving rows plus jobs-invariant task/scan hashes.
...
- Limits: manifest-list cardinality is still resident, engine preview parity and completion/schema evidence still need a bounded audit, retry/cancellation fault injection is incomplete, partition-evolution delete conformance needs a dedicated fixture, and performance/REST/live Glue evidence remain open in I2.
