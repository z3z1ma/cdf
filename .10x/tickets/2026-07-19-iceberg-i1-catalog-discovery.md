...
- 2026-07-19: Installed the ordinary registry `cdf add` hook without introducing catalog URI dialects or generic command logic. Iceberg claims a request only when `--option catalog=filesystem|rest|glue` is explicit; the positional value is the warehouse, REST URI, or Glue region, namespace is explicit (including exact JSON-array multipart form), table defaults to the resource-id suffix, selectors compile structurally, and cross-catalog/unknown options fail before discovery. The proposal is round-tripped through the same typed source/resource validators used by project compilation. Focused add tests, all 24 Iceberg tests, and strict all-target Clippy pass.
- 2026-07-19: Closed the catalog-semantic parity gap with a Glue-binding conformance test through the public driver boundary. An injected Glue `GetTable` pointer references a real metadata object consumed through neutral object access; its discovered Arrow schema, table UUID, and metadata generation exactly match filesystem discovery, its Glue version is retained as catalog generation, its database/table request is exact, and resolution reuses the prepared observation without a second Glue call. This isolates AWS control-plane mechanics from the common table semantics the three catalog bindings share.
- 2026-07-19: Adversarial plan-authority review found that sort-order metadata was validated in table metadata but not frozen into the external task-set authority. Added the complete hash-bound sort-order catalog and exact default order id once to shared authority, with structural/key/default validation and fixture assertions. This closes the plan contract without per-task amplification or runtime catalog rereads; the 25-test Iceberg suite and strict all-target Clippy pass.
