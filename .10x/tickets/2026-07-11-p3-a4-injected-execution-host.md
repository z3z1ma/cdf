...
- 2026-07-11: Completed composition-root service propagation across add, validate/deep, schema/promotion, state recovery, resume, replay, and doctor. Replaced discovery worker `futures_executor` memory waiting with a neutral thread-parking coordinator primitive. Added a permanent production-source gate rejecting runtime builders/singletons/private `block_on` outside the standalone host. Focused command tests, memory wake/release, checks, and strict Clippy pass; evidence is `.10x/evidence/2026-07-11-p3-a4-composition-and-runtime-gate.md`. Declared blocking-lane use and requirement admission remain before A4 closure.
