...
Make `cdf doctor` genuinely check configured Python interpreter health, version, GIL/free-threaded status, and `python.require_free_threaded` compatibility using only the configured interpreter process. Owns the Python doctor probe implementation in `crates/cdf-cli/**` and its focused tests.

## Acceptance Criteria

- `cdf doctor --json` reports a `python` check with structured details when `python.interpreter` is configured: resolved executable path, version, implementation, `gil_enabled`, `free_threaded_build`, `can_parallelize_python`, and `require_free_threaded`.
...
Record targeted `cdf-cli` tests for skipped, missing, invalid-output, GIL-enabled pass, and `require_free_threaded` failure cases; include a regression test proving `doctor` still does not create missing state or DuckDB files. Before closure, run focused fmt/test/clippy for `cdf-cli`, relevant workspace checks, security scans required by `QUALITY.md`, and update the observability parent with the child outcome.

## Explicit Exclusions

Do not implement Python resource execution, run orchestration, preview source opening, status freshness evaluation, OTLP export, or `inspect run`. Do not add a `cdf-python` dependency to `cdf-cli` unless implementation evidence shows the fixed process probe is insufficient.

## Progress and Notes

- 2026-07-06: Opened from `.10x/specs/project-cli-observability-security.md`, the observability parent ticket, and the existing `cdf-python` interpreter/free-threaded semantics. Current `cdf doctor` only checks that a configured interpreter path exists; this child turns that placeholder into a real doctor probe without crossing into blocked runtime orchestration.
- 2026-07-06: Implemented the `cdf-cli` process-based Python doctor probe with a fixed inline interpreter inspection script and no `cdf-python` dependency. Local worker checks passed `cargo fmt --all -- --check`, `cargo test -p cdf-cli --locked --no-fail-fast`, and `cargo clippy -p cdf-cli --all-targets --locked -- -D warnings`; parent verification/evidence/review remain outside this worker scope.
...
- 2026-07-06: Patched actionable mutation-test misses in `cdf-cli` tests: exact missing-interpreter branch wording, probe/setup failure details, inconsistent parseable probe JSON metadata, and free-threaded-build-with-GIL-enabled rejection. Re-ran minimum worker checks after the test additions.
