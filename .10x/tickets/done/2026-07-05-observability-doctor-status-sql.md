...
- 2026-07-12: `.10x/tickets/cancelled/2026-07-12-p3-j4-evidence-catalog-adbc.md` owns the next system-SQL expansion: read-only DataFusion catalog providers and bounded ADBC-compatible access over the existing ledger/package evidence stores. It reuses this parent rather than creating a second observability authority.
...
Closed on 2026-07-25. The acceptance surface is implemented through bounded terminal owners:

- run/resource/partition/package tracing and the runtime event spine;
- read-only local `cdf sql` over checkpoint/package/load evidence;
- structured `cdf doctor` environment, secrets, Python, DuckDB ICU, destination drift, and
  effective memory diagnostics;
- scheduler-friendly `cdf status` freshness and nonzero breach behavior;
- `cdf inspect run` plan/artifact/verdict/receipt/checkpoint story assembly with redaction.

The parked DataFusion catalog/ADBC expansion is a separate future query-access program and does
not keep this product surface open. OTLP export remains optional future integration rather than a
missing acceptance criterion.

## Review

Pass. Current CLI command modules and terminal child records cover every acceptance family without
source/destination-specific orchestration branches. Future probes remain owned by the adapter or
capability that introduces them.

## Retrospective

Doctor, status, SQL, tracing, and inspect evolved through separate bounded slices and should remain
separate extension surfaces. The broad parent was useful for bootstrapping but became stale once
those authorities and commands existed.
