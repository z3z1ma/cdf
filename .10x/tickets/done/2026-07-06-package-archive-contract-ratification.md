...
Ratify the persisted `cdf package archive` contract before implementing archive file writes, manifest mutation, or CLI behavior. This ticket owns shaping and spec updates only; it does not own source code changes.

## Record-backed context

The book and active package spec require `cdf package archive` to transcode `data/` Arrow IPC to Parquet with a fidelity report, keep Arrow IPC canonical, record both forms and hashes in package metadata, and prefer IPC for replay when present.
...
- The lifecycle behavior is explicit, including whether `cdf package archive` changes package status and how that interacts with replay preferring IPC when present.
