...
Owns the smallest necessary changes to `crates/firn-dest-postgres/**`, `crates/firn-conformance/**` only if the reusable destination harness needs a strictly general public helper, and this ticket's evidence/review records. Keep crate roots thin.

## Acceptance criteria

- `firn-dest-postgres` consumes `firn-conformance::destination::assert_destination_conformance` through a dev-dependency, matching the DuckDB and Parquet destination pattern.
...
- `cargo test -p firn-conformance --locked --no-fail-fast`
- `cargo clippy -p firn-dest-postgres --all-targets --locked -- -D warnings`
- `cargo clippy -p firn-conformance --all-targets --locked -- -D warnings`
- `cargo fmt --all -- --check`
