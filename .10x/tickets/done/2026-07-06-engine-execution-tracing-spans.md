...
Implement the first bounded tracing-field slice for engine package execution. Add an explicit caller-supplied `RunId` tracing entry point around `firn-engine` package execution so future run orchestration can emit spans with the book-required run, resource, partition, and package identifiers without the engine inventing run identity.

Own `crates/firn-engine/**`, `Cargo.lock`, `crates/firn-engine/Cargo.toml`, this ticket's evidence/review records, and parent progress notes. Keep `crates/firn-engine/src/lib.rs` thin; if a new type is needed, put it in an existing focused module or a new focused module rather than expanding the crate root.
...
- `firn-engine` exposes an explicit package execution entry point that requires a caller-supplied `RunId`.
...
Record focused `cargo fmt --all -- --check`, `git diff --check`, `cargo test -p firn-engine --locked --no-fail-fast`, and `cargo clippy -p firn-engine --all-targets --locked -- -D warnings`.
...
- `firn-the-book-of-the-system.md` Chapter 15 observability and Chapter 17 CLI.
