...
- 2026-07-19: The follow-up adversarial review found four remaining counterexamples and the protocol now closes them at their owning boundaries. Compiler artifact loading and semantic reconstruction are one injected operation that returns an opaque owned authority, so task claims cannot be paired with separately invented semantics. Artifact verification returns observed row facts and source verification returns independently observed position/schema/input-row/source-byte facts; coordinator admission compares those facts with every worker claim. A permit-owning write session enforces cumulative object/byte authority before invoking the mutation sink, and portable path validation now rejects POSIX, drive-qualified, UNC, extended Windows, and case-insensitive local-file URI forms.
- 2026-07-19: Final review exposed two last ambiguous-state paths. Quarantine artifacts now require independently observed row counts rather than defaulting missing facts to zero. The write session reserves count, bytes, and object identity before invoking the storage sink and never restores authority after an ambiguous provider error or invalid acknowledgement, preventing repeated post-mutation errors from exceeding the permit.
...
- `CARGO_BUILD_JOBS=12 cargo test -p cdf-runtime worker_protocol::tests --locked` — 9/9 passed after the second review repair. New counterexamples prove that valid artifact bytes carrying semantic identity Y cannot be admitted against task identity X; mutually rehashed receipt/result row lies fail against stored segment facts; a second individually legal write exceeding the cumulative permit fails before the mock sink mutates; and UNC, extended Windows, and uppercase `FILE:` paths fail on every host.
- `CARGO_BUILD_JOBS=12 cargo clippy -p cdf-runtime --all-targets --locked -- -D warnings` and `git diff --check` — passed after the second review repair.
- Isolated clean-tree verification at `029a1cbc`: `CARGO_BUILD_JOBS=12 cargo test -p cdf-runtime --lib --locked` — 97 passed, 1 explicit performance test ignored; `CARGO_BUILD_JOBS=12 cargo clippy -p cdf-runtime --all-targets --locked -- -D warnings` — passed. The detached worktree avoided consuming or validating the concurrent object-access/Iceberg lane's uncommitted files.
...
- 2026-07-19 — follow-up adversarial protocol/authority review at `3e46252c`: **fail**. Two critical counterexamples showed compiler semantic identity and stored segment row counts could still be laundered through mutually consistent claims. Two significant findings showed cumulative output limits were only rejected after side effects and path portability omitted UNC/case variants. All four are repaired above; final focused re-review pending.
- 2026-07-19 — final focused adversarial review at `029a1cbc`: **pass**. The reviewer verified opaque evidence-derived reconstruction, independent source/segment/quarantine facts, irrevocable pre-sink cumulative write reservation across ambiguous failures, and cross-host path rejection. No critical or significant finding remains. Residual provider-specific conditional-write atomicity belongs to the future storage substrate named by the protocol contract, not WX1's transport-neutral capsule.

## Retrospective

The central lesson was that a hash-shaped field is not authority merely because it validates syntactically or agrees with another claim. Portable execution authority must be reconstructed from verified compiler bytes as one operation, and result counts must be compared with independently observed storage/source facts. Likewise, a pre-write predicate is insufficient when provider failures are ambiguous: the attempt must consume cumulative authority before mutation and conservatively retain it after uncertainty. Keeping these rules in neutral protocol types lets every future worker substrate inherit them without source-, destination-, or scheduler-specific branches.

The adversarial review was valuable because each counterexample targeted a trust-boundary composition that ordinary happy-path fixtures could not falsify. The final suite now encodes those hostile combinations directly. WX1 intentionally stops at canonical capsule/admission authority; A7/A8 own stream policy/epochs, and C5 owns whole direct-versus-isolated execution equivalence.
