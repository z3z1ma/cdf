...
Owns `crates/cdf-conformance/**` for the reusable harness and `crates/cdf-formats/**` for the smallest production file-resource wrapper over the existing `read_file_source` implementation. The wrapper must reuse the current file readers and DuckDB-backed Parquet path; this ticket does not change the native Arrow/DataFusion Parquet policy.

Keep crate roots thin and use focused modules rather than expanding `lib.rs`.

## Acceptance criteria

- `cdf-conformance` exposes a reusable async execution conformance helper for `ResourceStream` candidates with representative `ScanRequest` cases.
...
- `cdf-formats` exposes a file-backed `ResourceStream` wrapper over `FileSource` without changing existing reader semantics, package semantics, or the DuckDB-backed Parquet supply-chain workaround.
...
- `cargo test -p cdf-conformance --locked resource -- --nocapture`
- `cargo test -p cdf-formats --locked --no-fail-fast`
- `cargo clippy -p cdf-conformance -p cdf-formats --all-targets --locked -- -D warnings`
- `cargo nextest run -p cdf-conformance -p cdf-formats --locked`
- `cargo fmt --all -- --check`
...
- `VISION.md` Chapters 7, 8, 9, 19, and 22.
...
- 2026-07-06: Split from the conformance parent after inspection showed the existing resource conformance harness is planning-only and `cdf-formats` has real file readers but no openable `ResourceStream` wrapper. The next complete step is an execution-level conformance oracle plus a real file-source consumer for CSV, JSON, NDJSON, and Parquet.
- 2026-07-06: Implemented `cdf-conformance::resource::assert_resource_stream_execution_conformance` with negative self-tests and added `cdf-formats::FileResource` over the existing file readers. CSV, JSON, NDJSON, and Parquet tests now consume both planning and execution conformance harnesses.
