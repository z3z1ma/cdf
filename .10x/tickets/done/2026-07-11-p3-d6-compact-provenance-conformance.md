...
- 2026-07-11: Closed D6 with the catalog-driven aggregate law and current adapter matrix. All enrolled destinations must publish supported persistence/targetability on the same kernel `RowProvenanceAddress`; individual leaf suites prove exact address lookup/correction, rollback, duplicate replay, missing/conflicting map rejection, range uniqueness, receipt binding, and physical-key opacity. DuckDB's current provenance path runs at 84.6% of raw Arrow appender throughput; PostgreSQL compact binary COPY runs at 2.64x CSV server-inclusive; Parquet adds no per-row provenance payload and binds its manifest sidecar to the receipt. Evidence: `.10x/evidence/2026-07-11-p3-d6-cross-adapter-closeout.md`; review: `.10x/reviews/2026-07-11-p3-d6-closeout-review.md`.
