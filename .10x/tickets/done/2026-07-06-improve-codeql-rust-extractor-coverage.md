...
Investigate and improve the local CodeQL Rust extractor setup so `codeql database analyze` reports high-confidence extraction metrics for the CDF Rust workspace. Owns local quality-tooling configuration and evidence only.
...
- 2026-07-06: Current refreshed metrics are 114 Rust files scanned, 81 with warnings, 33 without warnings, 1149 extraction warnings, 0 extraction errors, and 0 SARIF findings. The remaining warnings are ordinary macro expansion failures plus a CodeQL extractor-side `cargo metadata --lockfile-path` compatibility warning under local Cargo 1.96.1. No minimal safe CDF source change was identified that would materially improve extraction without rewriting normal Rust idioms.
