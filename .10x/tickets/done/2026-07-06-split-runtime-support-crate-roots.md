...
Refactor `crates/firn-state-sqlite/src/lib.rs` and `crates/firn-http/src/lib.rs` into focused internal modules without changing public crate-root APIs or behavior.
...
No changes outside `crates/firn-state-sqlite/**`, `crates/firn-http/**`, and this ticket's own evidence/review records.
...
- 2026-07-06: Split `firn-state-sqlite` and `firn-http` crate roots into focused internal modules with crate-root re-exports. Before/after root counts and module list recorded in `.10x/evidence/2026-07-06-runtime-support-crate-root-split.md`.
- 2026-07-06: Parent integration recheck passed `cargo fmt --all -- --check`, `cargo check --workspace --all-targets --locked`, `cargo test -p firn-state-sqlite -p firn-http --locked --no-fail-fast`, and `cargo clippy -p firn-state-sqlite -p firn-http --all-targets --locked -- -D warnings`.
