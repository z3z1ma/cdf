...
Implement the first reusable `firn-conformance` suite: a public checkpoint-store conformance harness that future store implementations can run against the same firn-line contract as the MVP in-memory and SQLite stores.

Owns `crates/firn-conformance/**`, the smallest necessary `crates/firn-state-sqlite/**` test integration, and its own evidence/review records. Keep `src/lib.rs` thin by splitting conformance support into focused modules.

## Acceptance criteria

- `firn-conformance` exposes a reusable checkpoint-store conformance function or harness that accepts a fresh-store factory and exercises the public `CheckpointStore` trait without depending on private test hooks.
...
Record targeted `cargo test -p firn-conformance --locked --no-fail-fast`, `cargo test -p firn-state-sqlite --locked --no-fail-fast`, `cargo clippy -p firn-conformance --all-targets --locked -- -D warnings`, `cargo clippy -p firn-state-sqlite --all-targets --locked -- -D warnings`, and `cargo fmt --all -- --check` output. Record broader quality checks required before parent commit.
...
- 2026-07-06: Split from the conformance parent after inspecting the checkpoint-store spec, existing in-memory/SQLite store tests, and the prior checkpoint-store review. This child lifts shared store-contract assertions into `firn-conformance` without changing product semantics.
- 2026-07-06: Worker A began implementation after reading the ticket, referenced specs/tickets/review, `crates/firn-conformance/**`, checkpoint store implementations/tests, and relevant kernel checkpoint/destination/id/position/scope types.
- 2026-07-06: Implemented public `firn_conformance::checkpoint_store` harness over `CheckpointStore`, added the `Send + Sync` helper, and wired both `InMemoryCheckpointStore` and `SqliteCheckpointStore` through the suite from `firn-state-sqlite` tests while preserving SQLite-only tests.
- 2026-07-06: Required verification passed: `cargo fmt --all -- --check`; `cargo test -p firn-conformance --locked --no-fail-fast`; `cargo test -p firn-state-sqlite --locked --no-fail-fast`; `cargo clippy -p firn-conformance --all-targets --locked -- -D warnings`; `cargo clippy -p firn-state-sqlite --all-targets --locked -- -D warnings`. Evidence recorded in `.10x/evidence/2026-07-06-checkpoint-store-conformance-suite.md`; review recorded in `.10x/reviews/2026-07-06-checkpoint-store-conformance-suite.md`.
