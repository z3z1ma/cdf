...
- 2026-07-19: `cdf-source-iceberg` passed 3 unit tests and its Arrow-58/engine-free and sibling-isolation graph laws. The broader runtime build-graph suite passed 6/7; its only failure was the separately owned and subsequently closed `.10x/tickets/done/2026-07-19-p0-runtime-egress-build-graph-regression.md`. A clean `HEAD` archive independently contained the same 85 packages against the existing 67-package ceiling, proving F2 did not cause or encode that regression.
