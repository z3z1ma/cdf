...
Implement the first fast-follow Tier 4 Singer/Airbyte source-adapter slice in `cdf-subprocess`: protocol message parsing, record-to-batch conversion, schema/catalog exposure where practical, and opaque protocol state mapping to `SourcePosition::ForeignState`.

Owns `crates/cdf-subprocess/**`, its manifest if a small JSON dependency change is required, and its own evidence/review records. It may add focused modules such as `singer.rs` or `airbyte.rs`; do not grow a monolithic `src/lib.rs`.
...
- Parser output can be written to and replayed from a CDF package using the existing package APIs.
...
Record tests for Singer schema/record/state parsing, Airbyte catalog/record/legacy-stream-global state parsing, malformed message handling, deterministic `ForeignState` hashes, package write/replay compatibility, formatting, clippy, and targeted tests for `cdf-subprocess`.

## Explicit exclusions

No `cdf package archive`, Parquet transcode, Airbyte destination support, full connector lifecycle, external process invocation contract changes, or state migration UI. Parquet archive work remains gated by `.10x/tickets/done/2026-07-06-parquet-format-source-supply-chain.md` or a later ratified policy/alternative writer.
...
- 2026-07-06: Worker subagent started implementation after reading the ticket, referenced research/spec/knowledge records, current `cdf-subprocess` files, `cdf-formats` read model, kernel batch/position types, and package writer/reader APIs. Existing dirty `.gitignore` is unrelated and must remain untouched.
