...
Implement fast-follow Singer and Airbyte source adapters over the subprocess machinery, `ForeignState` handling, protocol parsers, state mapping, and `cdf package archive` Parquet transcode with fidelity report. Owns parser additions in `cdf-subprocess`/`cdf-formats` and archive additions in `cdf-package`/CLI.
...
- 2026-07-06: Split `.10x/tickets/done/2026-07-06-singer-airbyte-protocol-adapters.md` for executable Singer/Airbyte protocol parsing and opaque `ForeignState` mapping. `cdf package archive` remains intentionally excluded from that child because Parquet transcode intersects the active supply-chain blocker.
- 2026-07-06: Closed `.10x/tickets/done/2026-07-06-singer-airbyte-protocol-adapters.md` with focused Singer/Airbyte parser, per-stream batch conversion, canonical opaque `ForeignState` hashing, package write/replay tests, mutation-clean adapter tests, and QUALITY evidence. This parent remains open for `cdf package archive`.
...
- 2026-07-06: Closed ratification ticket `.10x/tickets/done/2026-07-06-package-archive-contract-ratification.md` after updating `.10x/specs/package-lifecycle-determinism.md` with the persisted archive contract. Opened executable child `.10x/tickets/done/2026-07-06-package-archive-persistence-cli.md` for the source implementation.
- 2026-07-06: Closed package archive persistence child `.10x/tickets/done/2026-07-06-package-archive-persistence-cli.md` with persisted sidecars, manifest archive metadata, canonical fidelity report verification, status-gated write/skip/replace behavior, CLI archive parsing/output, focused tests, and QUALITY evidence.
- 2026-07-06: Parent closure reviewed in `.10x/reviews/2026-07-06-singer-airbyte-package-archive-parent-review.md`. The cross-cutting native Arrow/DataFusion Parquet policy question was tracked separately by `.10x/tickets/done/2026-07-06-native-arrow-parquet-policy.md` and was not a blocker for this parent contract. It was later ratified by `.10x/decisions/native-arrow-datafusion-parquet-policy.md`.
