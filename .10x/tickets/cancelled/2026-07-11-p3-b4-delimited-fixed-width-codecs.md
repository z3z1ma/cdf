...
- 2026-07-19: Falsified and removed the first quote-aware inner-unit design rather than weakening the performance floor. On the controlled EC2 host, its required boundary pre-scan made one file 75.3% slower (1.819529s versus 1.037673s) and four files 2.5% slower (3.598940s versus 3.511328s). It also changed segment row counts and the package hash, proving that generic decode-unit boundaries leaked into identity-bearing segmentation. Commit `fa8163f1` removes the entire candidate surface, including the speculative source capability and format knobs. A future design must be single-pass and must carry canonical row segmentation across decode units; the current sequential path remains the proven default.
...
- 2026-07-19 rejected quote-aware unit candidate:
  - Clean EC2 revision `a8a5fa3e`: one-file sequential 1.037673s versus units 1.819529s; four-file sequential 3.511328s versus units 3.598940s.
  - The one-file package hash changed from `sha256:d5969a5c36040a9e14496aaff00b632461af37c967eb1741cab08b303ea1807c` to `sha256:d5369d7689bd4abb72d35c1c4000b105d84d05bee291e60057fc12fec9c6183c`; segment row counts changed at unit boundaries. This falsifies both the no-regression and jobs/strategy-invariance requirements.
  - `git revert` commit `fa8163f1` deletes the candidate. The result is a null hypothesis, not deferred legacy code.
