...
Created: 2026-07-08
Updated: 2026-07-25
Parent: .10x/tickets/cancelled/2026-07-05-wasm-components-registry-signing.md
...
- 2026-07-08: Split from `.10x/tickets/cancelled/2026-07-05-wasm-components-registry-signing.md` because the full WASM/registry/signing ticket is too broad to execute as one slice. This child only makes the WIT interface artifact concrete and reviewable.
...
- `rustfmt --edition 2024 --check crates/cdf-wasm/src/lib.rs` passed after applying the formatter's one source-only layout correction. `git diff --check -- crates/cdf-wasm .10x/tickets/cancelled/2026-07-08-wasm-wit-interface-foundation.md` passed.
...
Parked on 2026-07-25. The rejected draft left no WIT, dependency, lockfile, or runtime residue.
Recursive kernel values, typed control/terminal ordering, cancellation, broker calls, and the
current WASI/Wasmtime lifecycle require one future ratified interface program before a versioned
artifact is published. Research and review remain the reconstruction authority.
