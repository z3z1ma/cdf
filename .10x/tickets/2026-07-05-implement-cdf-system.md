...
- 2026-07-07: Closed `.10x/tickets/done/2026-07-07-cli-resume-spine.md`; `cdf resume` now recovers run-id-scoped interrupted work through package/receipt/checkpoint facts across DuckDB, filesystem Parquet, and Postgres without source contact after package finalization. The CLI spine parent remains open for direct CLI table-backed SQL `run` success evidence in `.10x/tickets/2026-07-07-cli-sql-run-success.md`.
