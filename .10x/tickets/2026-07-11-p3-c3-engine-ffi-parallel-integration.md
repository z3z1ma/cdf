...
- 2026-07-14 teardown correction: final review found a broader ownership path than the repaired internal future cycle: legal user work can capture the final `Arc<StandaloneExecutionHost>` or services, causing `FixedTaskPool::drop` to run on one of its own workers. Pool teardown now identifies and detaches only the current worker handle while still joining every other worker; the current loop completes its item and exits through the already-set shutdown state. A focused regression deliberately makes a worker release the final pool owner and requires completion within one second; it passed 20 consecutive runs, the 22-test host set passed, and strict engine Clippy remained clean.
...
- 2026-07-14 final-review interim finding — **significant**. User tasks could own the final host/pool reference and invoke pool destruction on their own worker. Self-safe teardown and a direct last-owner regression now repair this path; the reviewer is continuing against the amended range.
