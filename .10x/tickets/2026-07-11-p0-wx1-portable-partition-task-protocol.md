...
- `CARGO_BUILD_JOBS=12 cargo test -p cdf-runtime worker_protocol::tests --locked` — 5/5 passed. Covers the checked-in canonical task fixture, round trip and hash stability, task/result tampering, unsupported compatibility, stale/expired fencing, nonidentity telemetry, explicit metadata-budget failure, output-scope authorization, and absence of payload/commit authority fields.
