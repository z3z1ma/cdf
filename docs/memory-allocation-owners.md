...
| cdf-source-mongodb | mongodb-arrow-decode | Decode | MONGODB_MAXIMUM_DECODE_BYTES | crates/cdf-source-mongodb/src/execution.rs | 1 |
...
| MongoDB source client and raw BSON cursor internals | native | cdf-source-mongodb one-query official client session | one reusable client pool capped at the selected size of one, a 64 MiB raw BSON poll lease, and a 128 MiB decode lease covering construction scratch plus retained Arrow/drift evidence before a maximum 64 MiB batch crosses the source frontier | .10x/evidence/2026-08-04-mongodb-source-connector.md | bounded |
