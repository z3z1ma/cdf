...
| `CDF-ENV-HOST` | ENV | environment | 70 | host | Restore the required host or process facility, then retry. Check the path, permissions, temporary directory, executable availability, and process resource limits named by the error. | `cdf doctor` |
...
| `CDF-PROJECT-RESOURCE-ID` | PROJECT | contract | 3 | resource id | Choose a project resource id that does not collide with an existing resource. Run `cdf inspect resources` to review current project resource ids. | `cdf validate` |
| `CDF-RESOURCE-NOT-COMPILED` | RESOURCE | contract | 3 | not compiled | Use a compiled resource id or author the expected project SQL resource. Run `cdf inspect resources` to see compiled ids and their project SQL files. If the id is expected, author `cdf/<namespace>/<resource>.cdf.sql` with an explicit configured-source binding. | `cdf inspect resources` |
...
| `CDF-RUN-PACKAGE-ARGUMENT` | RUN | contract | 2 | package argument | Provide the package destination arguments required by the destination. For Postgres package execution, pass `--target schema.table`. | `cdf run` |
| `CDF-RUN-PACKAGE-CONTRACT` | RUN | contract | 3 | package contract | Make run arguments match the package delivery contract. Use the package destination target recorded at package creation time. | `cdf run` |
...
| `CDF-SOURCE-REFERENCE` | SOURCE | contract | 3 | reference | Repair the registered source-driver configuration or resource target, then retry. Run `cdf doctor` to execute the source driver's bounded health probe without reading resource payloads. | `cdf help` |
...
| `CDF-STATE-RESUME-MULTI-RUN-NOT-SUPPORTED` | STATE | contract | 78 | resume multi run not supported | Resume one interrupted run explicitly. Pass `cdf run --resume RUN_ID` for the run you want to drain. | `cdf state show` |
