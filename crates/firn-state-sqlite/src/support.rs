...
use firn_kernel::{
    CHECKPOINT_STATE_VERSION, Checkpoint, CheckpointId, CheckpointStatus, FirnError, PackageHash,
...
pub(crate) fn missing_checkpoint(checkpoint_id: &CheckpointId) -> FirnError {
...
pub(crate) fn sqlite_error(error: rusqlite::Error) -> FirnError {
    FirnError::internal(error.to_string())
}

pub(crate) fn lock_error<T>(error: std::sync::PoisonError<T>) -> FirnError {
    FirnError::internal(error.to_string())
}
