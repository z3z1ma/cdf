    ...
    macro_rules! primitive {
        ($array_ty:ty, $value:expr, $key:expr) => {{
            let array = array.as_any().downcast_ref::<$array_ty>().ok_or_else(|| {
                FirnError::internal(format!("Arrow array downcast failed for {data_type:?}"))
            })?;
    ...
    match data_type {
        ...
        DataType::Utf8 => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow Utf8 downcast failed"))?;
        ...
        DataType::LargeUtf8 => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow LargeUtf8 downcast failed"))?;
            let value = array.value(row).to_owned();
            Ok(CellValue {
                value: Value::Text(value.clone()),
                key: CellKey::Text(value),
        ...
        DataType::Binary => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow Binary downcast failed"))?;
        ...
        DataType::LargeBinary => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow LargeBinary downcast failed"))?;
            let value = array.value(row).to_vec();
            Ok(CellValue {
                value: Value::Blob(value.clone()),
                key: CellKey::Blob(value),
        ...
        DataType::Time32(TimeUnit::Second) => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow Time32Second downcast failed"))?;
            let value = array.value(row);
        ...
        DataType::Time32(TimeUnit::Millisecond) => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow Time32Millisecond downcast failed"))?;
            let value = array.value(row);
        ...
        DataType::Time64(TimeUnit::Nanosecond) => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow Time64Nanosecond downcast failed"))?;
            let value = array.value(row);
            if value % 1_000 != 0 {
        ...
        DataType::Timestamp(TimeUnit::Nanosecond, None) => {
            let array = array
                ...
                .ok_or_else(|| FirnError::internal("Arrow TimestampNanosecond downcast failed"))?;
            let value = array.value(row);
            if value % 1_000 != 0 {
                return Err(FirnError::contract(
        ...
        DataType::Timestamp(_, Some(_)) => Err(FirnError::contract(
