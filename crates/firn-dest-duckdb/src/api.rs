...
impl DuckDbDestination {
    pub fn new(database_path: impl AsRef<Path>) -> Result<Self> {
        ...
        if database_path.as_os_str().is_empty() {
            return Err(FirnError::contract("DuckDB database path cannot be empty"));
        }
    ...
    pub fn verify_receipt(&self, receipt: &Receipt) -> Result<ReceiptVerification> {
        ...
        let stored_json: Option<String> = conn
            .query_row(
                "SELECT receipt_json FROM _firn_loads WHERE target = ? AND idempotency_token = ? AND package_hash = ?",
                params![
        ...
        let Some(stored_json) = stored_json else {
            return Ok(ReceiptVerification {
                ...
                reason: Some("receipt is absent from _firn_loads".to_owned()),
            });
        };
    ...
    pub fn probe_icu(&self) -> Result<IcuProbe> {
        let statement = "SELECT length(icu_sort_key('Firn', 'en_US')) > 0".to_owned();
        let conn = self.open_connection()?;
