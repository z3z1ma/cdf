...
use firn_package::{MANIFEST_FILE, read_manifest, read_receipts};
...
fn json_scalar_string<T: Serialize>(value: &T) -> Result<String, CliError> {
    ...
    value
        .as_str()
        .map(ToOwned::to_owned)
        .ok_or_else(|| CliError::from(FirnError::data("expected JSON string scalar")))
}

fn to_i64(value: impl TryInto<i64>) -> Result<i64, CliError> {
    value
        .try_into()
        .map_err(|_| CliError::from(FirnError::internal("integer does not fit in i64")))
}
...
fn json_cli_error(error: serde_json::Error) -> CliError {
    CliError::from(FirnError::data(error.to_string()))
}
