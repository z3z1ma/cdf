...
use firn_dest_duckdb::{DuckDbDestination, DuckDbMirrorLoadRow, DuckDbMirrorStateRow};
use firn_kernel::{FirnError, Receipt, StateDelta};
...
fn read_committed_heads(path: &PathBuf) -> Result<Vec<LedgerHead>, CliError> {
    ...
    let has_checkpoints = conn
        .query_row(
            "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'firn_checkpoints'",
            [],
            |_| Ok(()),
        )
        .optional()
    ...
    for (key, expected) in expected {
        let Some(actual) = actual.get(key) else {
            ...
            push_example(
                ...
                DriftExample {
                    kind: "missing_load",
                    reason: "no _firn_loads row for committed ledger receipt".to_owned(),
            ...
            if !matches {
                ...
                push_example(
                    ...
                    DriftExample {
                        ...
                        reason: "_firn_loads field differs from committed ledger receipt"
                            .to_owned(),
                        checkpoint_id: Some(expected.checkpoint_id.clone()),
                        target: Some(key.target.clone()),
                        idempotency_token: Some(key.idempotency_token.clone()),
    ...
    for (key, expected) in expected {
        let Some(actual) = actual.get(key) else {
            ...
            push_example(
                ...
                DriftExample {
                    kind: "missing_state",
                    reason: "no _firn_state row for committed ledger segment".to_owned(),
                    checkpoint_id: Some(expected.checkpoint_id.clone()),
                    target: Some(key.target.clone()),
            ...
            if !matches {
                ...
                push_example(
                    ...
                    DriftExample {
                        ...
                        reason: "_firn_state field differs from committed ledger segment"
                            .to_owned(),
                        checkpoint_id: Some(expected.checkpoint_id.clone()),
                        target: Some(key.target.clone()),
                        idempotency_token: None,
...
fn record_extra_loads(
    ...
    for key in actual.difference(&expected) {
        ...
        push_example(
            ...
            DriftExample {
                kind: "extra_load",
                reason: "_firn_loads row has no committed local ledger head".to_owned(),
                checkpoint_id: None,
                target: Some(key.target.clone()),
                idempotency_token: Some(key.idempotency_token.clone()),
                package_hash: None,
...
fn record_extra_states(
    ...
    for key in actual.difference(&expected) {
        ...
        push_example(
            ...
            DriftExample {
                kind: "extra_state",
                reason: "_firn_state row has no committed local ledger segment".to_owned(),
                checkpoint_id: None,
                target: Some(key.target.clone()),
                idempotency_token: None,
                package_hash: Some(key.package_hash.clone()),
...
fn sqlite_error(error: rusqlite::Error) -> CliError {
    CliError::from(FirnError::data(format!(
