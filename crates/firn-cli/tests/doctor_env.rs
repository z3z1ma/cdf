...
fn doctor_resolves_child_process_env_secrets_without_leaking_values() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_firn-cli"))
        .args(["--json", "--project", project.root_str(), "doctor"])
        .env(
            "FIRN_CLI_ENV_DESTINATION_DSN",
            "resolved-env-destination-value",
        )
        .env("FIRN_CLI_ENV_AUTH_TOKEN", "resolved-env-auth-token-value")
        .output()
        .unwrap();
    ...
    for reference in [
        "secret://env/FIRN_CLI_ENV_DESTINATION_DSN",
        "secret://env/FIRN_CLI_ENV_AUTH_TOKEN",
        "secret://file/sql-dsn",
    ] {
        assert!(
...
destination = "postgres://secret://env/FIRN_CLI_ENV_DESTINATION_DSN"
...
auth = { kind = "bearer", token = "secret://env/FIRN_CLI_ENV_AUTH_TOKEN" }
