...
pub const DLT_METADATA_ATTR: &str = "__firn_dlt_metadata__";
...
impl DltWriteDisposition {
    fn to_firn(&self) -> Option<WriteDisposition> {
        match self {
...
impl DltShimMetadata {
    ...
    pub fn migration_table(&self) -> DltShimMigrationTable {
        ...
        table.preview_only(
            ...
            "Firn preview consumes explicit metadata fixtures and does not import or delegate to a live dlt runtime.",
        );
        table.preview_only(
            "dlt.current.state writes",
            "CheckpointStore committed-head view",
            "Firn exposes current committed state by scope; staged state changes are checkpoint deltas and still require package receipts before becoming visible.",
        );
        table.unsupported(
            "dlt destination delegation",
            "firn destination protocol",
            "Firn does not call dlt destinations; batches leave Python through Firn descriptors and normal package/checkpoint flow.",
        );
        ...
        if let Some(incremental) = &self.incremental {
            table.mapped(
                ...
                format!(
                    "Cursor field `{}` maps to a Firn cursor with {:?} ordering and {} ms lag.",
            ...
            if incremental.initial_value.is_some() || incremental.end_value.is_some() {
                table.preview_only(
                    ...
                    "Initial and end bounds are recorded as preview metadata; Firn checkpoint commits still own durable cursor advancement.",
                );
            }
        }
        if let Some(disposition) = &self.write_disposition {
            match disposition.disposition.to_firn() {
                Some(_) => table.mapped(
                    "write_disposition",
                    "ResourceDescriptor.write_disposition",
                    format!(
                        "dlt `{}` maps to the same Firn disposition.",
                        disposition.disposition.as_str()
                    ),
                ),
                None => table.unsupported(
                    "write_disposition=skip",
                    "no Firn write disposition",
                    "Skipped dlt resources are not converted into Firn resources in this preview.",
                ),
            }
            if let Some(strategy) = &disposition.strategy {
                table.preview_only(
                    ...
                    format!(
                        "dlt strategy `{strategy}` is documented but not emulated by the Firn preview."
                    ),
                );
            }
        }
        if let Some(contract) = &self.schema_contract {
            table.mapped(
                ...
                format!(
                    "dlt contract mode `{}` maps to a deterministic Firn contract reference.",
                    contract.mode
                ),
            );
            for mode in &contract.unsupported_modes {
                table.preview_only(
                    ...
                    "migration-table note",
                    format!("dlt contract mode `{mode}` has no Firn preview equivalent."),
                );
            }
        }
        table
    ...
    pub fn apply_to_descriptor(&self, descriptor: &mut ResourceDescriptor) -> Result<()> {
        if !self.selected {
            return Err(FirnError::contract(
                "dlt resource is not selected and cannot be previewed as a Firn resource",
            ));
        }
        if let Some(disposition) = &self.write_disposition {
            descriptor.write_disposition = disposition.disposition.to_firn().ok_or_else(|| {
                FirnError::contract(
                    "dlt write_disposition=skip has no Firn resource descriptor mapping",
                )
            })?;
        }
        if let Some(primary_key) = &self.primary_key {
...
pub fn extract_dlt_metadata(object: &Bound<'_, PyAny>) -> Result<Option<DltShimMetadata>> {
    ...
    if object.hasattr("__firn_resource__").map_err(py_error)? {
        return extract_firn_resource_metadata(object).map(Some);
    }
    Ok(None)
}
...
pub fn dlt_current_state_view(
    ...
    Ok(DltCurrentStateView {
        ...
        source_state,
        writable: true,
        note: "Preview state is a view over committed Firn checkpoint heads; writes become durable only through checkpoint deltas committed with receipts.".to_owned(),
    })
}
...
fn extract_firn_resource_metadata(object: &Bound<'_, PyAny>) -> Result<DltShimMetadata> {
    Ok(DltShimMetadata {
        kind: DltShimObjectKind::Resource,
        name: string_attr(object, "__firn_name__")?,
        table_name: None,
        source_name: None,
        primary_key: Some(tuple_attr(object, "__firn_primary_key__")?),
        merge_key: Some(tuple_attr(object, "__firn_merge_key__")?),
        incremental: string_attr(object, "__firn_cursor__")?.map(|cursor_path| {
            DltIncrementalHint {
                cursor_path,
        ...
        selected: true,
        parallelized: bool_attr(object, "__firn_parallel__")?.unwrap_or(false),
    })
}
...
fn position_to_dlt_state(position: &SourcePosition) -> Result<Value> {
    match position {
        ...
        SourcePosition::FileManifest(manifest) => {
            serde_json::to_value(manifest).map_err(|error| FirnError::data(error.to_string()))
        }
    }
}
...
fn parse_incremental(raw: RawIncremental) -> Result<DltIncrementalHint> {
    let cursor_path = raw
        .cursor_path
        .or(raw.field)
        .ok_or_else(|| FirnError::contract("dlt incremental metadata must include cursor_path"))?;
...
fn optional_string(value: Option<Value>) -> Result<Option<String>> {
    match value {
        ...
        Some(other) => Err(FirnError::contract(format!(
...
pub fn fixture_dlt_foreign_state(state: &Value) -> Result<SourcePosition> {
    let opaque_blob =
        serde_json::to_vec(state).map_err(|error| FirnError::data(error.to_string()))?;
