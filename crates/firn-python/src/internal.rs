...
pub(crate) fn write_ipc_hash(batch: &RecordBatch, hasher: &mut Sha256) -> Result<()> {
    ...
    {
        let mut writer =
            StreamWriter::try_new(&mut bytes, batch.schema().as_ref()).map_err(FirnError::from)?;
...
pub(crate) fn same_path(expected: &Path, actual: &Path) -> Result<bool> {
    let expected = expected.canonicalize().map_err(|error| {
        FirnError::contract(format!("configured Python interpreter path error: {error}"))
    })?;
    let actual = actual.canonicalize().map_err(|error| {
        FirnError::contract(format!("attached Python interpreter path error: {error}"))
    })?;
    Ok(expected == actual)
}
...
pub(crate) fn py_error(error: PyErr) -> FirnError {
