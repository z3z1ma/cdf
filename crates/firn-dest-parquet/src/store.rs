...
impl StoreClient {
    pub(crate) fn new_filesystem(root: &Path) -> Result<Self> {
        fs::create_dir_all(root).map_err(|error| {
            FirnError::destination(format!("create {}: {error}", root.display()))
        })?;
        let store = LocalFileSystem::new_with_prefix(root).map_err(|error| {
            FirnError::destination(format!("open object store filesystem: {error}"))
        })?;
    ...
    pub(crate) fn get_required(&self, runtime: &Runtime, key: &str) -> Result<Vec<u8>> {
        self.get_optional(runtime, key)?
            .ok_or_else(|| FirnError::data(format!("object {key} is missing")))
    }
    ...
    fn path(&self, key: &str) -> Result<ObjectPath> {
        if key.trim().is_empty() {
            return Err(FirnError::contract("object key cannot be empty"));
        }
...
pub(crate) fn package_manifest_key(
    target: &TargetName,
    token: &firn_kernel::IdempotencyToken,
) -> String {
    format!(
...
pub(crate) fn segment_object_key(
    target: &TargetName,
    token: &firn_kernel::IdempotencyToken,
    segment_id: &firn_kernel::SegmentId,
) -> String {
    format!(
...
fn store_error(action: impl Into<String>, error: object_store::Error) -> FirnError {
    FirnError::destination(format!("{}: {error}", action.into()))
}

pub(crate) fn now_ms() -> Result<i64> {
    let duration = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|error| FirnError::internal(format!("system clock before epoch: {error}")))?;
    i64::try_from(duration.as_millis())
        .map_err(|_| FirnError::internal("system time does not fit i64 milliseconds"))
}
