...
pub(crate) fn verify_receipt(destination: &ParquetDestination, receipt: &Receipt) -> Result<()> {
    ...
    let manifest_key = receipt
        ...
        .parameters
        .get("manifest_key")
        .ok_or_else(|| FirnError::data("receipt is missing manifest_key verify parameter"))?;
    let expected_manifest_sha = receipt
        .verify
        .parameters
        .get("manifest_sha256")
        .ok_or_else(|| FirnError::data("receipt is missing manifest_sha256 verify parameter"))?;
    ...
    let manifest: ParquetObjectManifest =
        serde_json::from_slice(&manifest_bytes).map_err(|error| {
            FirnError::data(format!("parse Parquet manifest {manifest_key}: {error}"))
        })?;
    ...
    if receipt.disposition == WriteDisposition::Replace {
        let transaction = receipt
            .transaction
            .as_ref()
            .ok_or_else(|| FirnError::data("replace receipt is missing transaction metadata"))?;
        let pointer_key = transaction
            .values
            .get("replace_pointer_key")
            .ok_or_else(|| FirnError::data("replace receipt is missing replace_pointer_key"))?;
        let pointer_sha256 = transaction
            .values
            .get("replace_pointer_sha256")
            .ok_or_else(|| FirnError::data("replace receipt is missing replace_pointer_sha256"))?;
        let bytes = destination
            .store()
        ...
        let parsed: ReplacePointer = serde_json::from_slice(&bytes).map_err(|error| {
            FirnError::data(format!("parse replace pointer {pointer_key}: {error}"))
        })?;
...
fn segment_acks(manifest: &ParquetObjectManifest) -> Result<Vec<SegmentAck>> {
    manifest
        ...
        .map(|object| {
            Ok(SegmentAck {
                segment_id: firn_kernel::SegmentId::new(object.segment_id.clone())?,
