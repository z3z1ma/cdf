...
const SEGMENT_TABLE: &str = "firn_segment";
...
pub(crate) fn write_parquet_segment(segment: &LoadedSegment) -> Result<Vec<u8>> {
    ...
    append_batches(&conn, &segment.batches)?;
    ...
    let parquet_path = parquet_path
        .to_str()
        .ok_or_else(|| FirnError::destination("temporary Parquet path is not valid UTF-8"))?;
    ...
    for field in fields {
        if !seen.insert(field.name()) {
            return Err(FirnError::contract(format!(
                "duplicate Parquet destination column name {}",
                field.name()
            )));
        }
    }
...
fn duckdb_error(context: impl Into<String>, error: duckdb::Error) -> FirnError {
