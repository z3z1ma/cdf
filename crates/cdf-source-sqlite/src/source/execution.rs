...
use cdf_kernel::{
    Batch, BatchId, CdfError, CursorPosition, EffectiveSchemaRuntime,
...
use rusqlite::{Connection, OpenFlags, TransactionBehavior, params_from_iter, types::ValueRef};
...
use super::{
    query::{SqliteTableScan, build_query, projected_fields, scan_from_partition},
...
pub(super) const SQLITE_FETCH_ROWS: usize = 32 * 1024;
...
pub(super) struct SqliteExecutionInput {
    ...
    pub(super) table: SqliteIdentifier,
...
pub(super) fn execute_sqlite_table(
    ...
    .map_err(|error| {
        classify_sqlite_open_error("open SQLite source database", &input.database_path, error)
    })?;
    connection
    ...
    if let Some(stable_key) = input.stable_key.as_ref() {
        validate_live_unique_stable_key(&transaction, &input.table, stable_key)?;
    }
    ...
    loop {
        ...
        let mut builders = projected_fields
            .iter()
            .map(|field| ColumnBuilder::new(field))
            .collect::<Result<Vec<_>>>()?;
        let mut row_count = 0_usize;
        let mut estimated_bytes = 0_u64;
        let mut maximum_cursor = None::<ObservedCursor>;
        while row_count < SQLITE_FETCH_ROWS && estimated_bytes < SQLITE_TARGET_BATCH_BYTES {
            ...
            for (index, (field, builder)) in projected_fields.iter().zip(&mut builders).enumerate()
            {
                let value = row
                    .get_ref(index)
                    .map_err(|error| classify_sqlite_error("read SQLite source value", error))?;
                let value_bytes =
                    fixed_value_bytes[index].unwrap_or_else(|| estimated_value_bytes(value));
                ...
                if value_bytes > SQLITE_MAXIMUM_BATCH_BYTES
                    || next_estimated_bytes > SQLITE_MAXIMUM_BATCH_BYTES
                {
...
fn execution_physical_schema(
    ...
    let live = discover_sqlite_table_on_connection(
        connection,
        &input.descriptor.resource_id,
        &input.table,
    )?;
    let projected = project_physical_schema(&live.schema, &input.schema, &scan.projection)?;
    ...
    let live_hash = cdf_kernel::canonical_arrow_schema_hash(&live.schema)?;
    if live_hash != observation.physical_schema_hash {
        return Err(CdfError::data(format!(
            "SQLite table `{}` physical schema changed after verified discovery; expected {}, observed {live_hash}; refresh discovery before retrying",
            input.table, observation.physical_schema_hash
        )));
    }
    if live.schema != **expected_physical {
        return Err(CdfError::data(format!(
            "SQLite table `{}` live physical schema does not match verified catalog entry {}",
            input.table, observation.physical_schema_hash
        )));
    }
    ...
    pub(super) fn new(field: &Field) -> Result<Self> {
        Ok(match field.data_type() {
            ...
            DataType::Timestamp(TimeUnit::Second, timezone) => {
                Self::TimestampSecond(TimestampSecondBuilder::new(), timezone.clone())
            }
            DataType::Timestamp(TimeUnit::Millisecond, timezone) => {
                Self::TimestampMillisecond(TimestampMillisecondBuilder::new(), timezone.clone())
            }
            DataType::Timestamp(TimeUnit::Microsecond, timezone) => {
                Self::TimestampMicrosecond(TimestampMicrosecondBuilder::new(), timezone.clone())
            }
            DataType::Timestamp(TimeUnit::Nanosecond, timezone) => {
                Self::TimestampNanosecond(TimestampNanosecondBuilder::new(), timezone.clone())
            }
            other => {
                return Err(CdfError::data(format!(
