...
pub use firn_package::{PackageBuilder, PackageReader};
pub use firn_project::{PreparedDuckDbReplayReport, PreparedReceiptSource};
pub use firn_state_sqlite::SqliteCheckpointStore;
...
pub fn assert_duckdb_mirror_matches_receipt(
    ...
    assert!(snapshot.loads_table_present, "_firn_loads must exist");
    assert!(snapshot.state_table_present, "_firn_state must exist");
    ...
    let load = snapshot
        ...
        .expect("DuckDB _firn_loads must contain the durable receipt mirror");
    ...
    assert_eq!(
        ...
        "DuckDB _firn_state must mirror every replayed segment exactly once"
    );
