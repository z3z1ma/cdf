...
fn assert_batch_header_and_payload(
    ...
    batch: &Batch,
    seen_batch_ids: &mut BTreeSet<firn_kernel::BatchId>,
) -> u64 {
    assert_eq!(
...
mod tests {
    ...
    use firn_kernel::{
        BatchId, BatchPayload, BatchStats, BoxFuture, FilePosition, FirnError, PayloadRef,
    ...
    impl ResourceStream for FaultyExecutionResource {
        ...
        fn open(
            &self,
            partition: PartitionPlan,
        ) -> BoxFuture<'_, Result<firn_kernel::BatchStream>> {
            let fault = self.fault;
            Box::pin(async move {
                let batches = batches_for_partition(&partition.partition_id, fault)?;
                Ok(Box::pin(stream::iter(batches.into_iter().map(Ok))) as firn_kernel::BatchStream)
            })
        }
    }
    ...
    fn batches_for_partition(
        ...
        let rows = match partition_id.as_str() {
            "file-part-0" => [1, 2],
            "file-part-1" => [3, 4],
            other => return Err(FirnError::internal(format!("unexpected partition {other}"))),
        };
