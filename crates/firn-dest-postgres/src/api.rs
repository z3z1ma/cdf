...
pub fn build_receipt(plan: &PostgresLoadPlan, input: PostgresReceiptInput) -> Result<Receipt> {
    ...
    transaction_values.insert("loads_table".to_owned(), FIRN_LOADS_TABLE.to_owned());
    transaction_values.insert("state_table".to_owned(), FIRN_STATE_TABLE.to_owned());
    ...
    Ok(Receipt {
        ...
        package_hash: PackageHash::new(
            plan.verify
                .parameters
                .get("package_hash")
                .ok_or_else(|| FirnError::internal("verify clause missing package_hash"))?
                .clone(),
        )?,
        ...
        idempotency_token: IdempotencyToken::new(
            plan.verify
                ...
                .ok_or_else(|| FirnError::internal("verify clause missing idempotency_token"))?
                .clone(),
        )?,
        ...
        schema_hash: SchemaHash::new(
            plan.verify
                .parameters
                .get("schema_hash")
                .ok_or_else(|| FirnError::internal("verify clause missing schema_hash"))?
                .clone(),
        )?,
...
pub fn source_exercise_hooks(
    ...
    Ok(PostgresSourceExerciseHooks {
        snapshot_count: PostgresStatement::query(
            ...
            format!(
                "SELECT COUNT(*) AS \"firn_source_rows\" FROM {}",
                target.sql()
            ),
            StatementExpectation::ReturnsMirrorRows,
        ),
