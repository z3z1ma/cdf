...
fn identifiers_quote_safely_and_reject_reserved_user_names() {
    ...
    assert!(PostgresIdentifier::user("_firn_load").is_err());
...
fn append_replace_and_merge_plans_include_transactional_sql() {
    ...
    assert!(
        append
            .system_ddl
            .iter()
            .any(|statement| statement.sql.contains(FIRN_LOADS_TABLE))
    );
    assert!(
        append.target_ddl[0]
            .sql
    ...
    assert!(merge_sql.contains("ORDER BY \"_firn_segment\" DESC, \"_firn_row\" DESC"));
...
fn merge_dedup_policy_is_explicit_and_can_fail_on_duplicates() {
    ...
    let first_sql = first
        ...
        .sql
        .as_str();
    assert!(first_sql.contains("ORDER BY \"_firn_segment\" ASC, \"_firn_row\" ASC"));
...
fn receipt_contains_postgres_xid_verify_clause_and_segment_acks() {
    ...
    assert!(receipt.verify.statement.contains(FIRN_LOADS_TABLE));
...
fn mirror_and_drift_hooks_expose_load_and_state_tables() {
    ...
    assert!(
        plan.mirror_sql
            .iter()
            .any(|statement| statement.sql.contains(FIRN_STATE_TABLE))
    );
    assert!(plan.drift.load_for_package.sql.contains(FIRN_LOADS_TABLE));
    assert!(plan.drift.state_for_scope.sql.contains(FIRN_STATE_TABLE));
    assert_eq!(
...
fn existing_table_migrations_add_only_safe_missing_columns() {
    ...
    assert!(plan.target_ddl.iter().any(|statement| statement.sql
        == "ALTER TABLE \"raw\".\"orders\" ADD COLUMN \"_firn_load\" TEXT"));
