...
impl PostgresDestination {
    pub fn connect(database_url: impl Into<String>) -> Result<Self> {
        ...
        if database_url.trim().is_empty() {
            return Err(FirnError::contract("Postgres database URL cannot be empty"));
        }
        Ok(Self {
...
fn find_duplicate_receipt(
    ...
    let row = tx
        ...
        .map_err(|error| postgres_error("query Postgres _firn_loads idempotency", error))?;
    row.map(|row| {
...
fn count_target_rows(tx: &mut Transaction<'_>, target: &PostgresTarget) -> Result<u64> {
    ...
    u64::try_from(count).map_err(|_| FirnError::internal("Postgres count was negative"))
}
...
fn count_merge_source_rows(tx: &mut Transaction<'_>, plan: &PostgresLoadPlan) -> Result<u64> {
    let sql = match plan.dedup {
        MergeDedupPolicy::First | MergeDedupPolicy::Last => format!(
            "{}SELECT COUNT(*)::bigint FROM \"_firn_dedup\"",
...
fn count_merge_updates(tx: &mut Transaction<'_>, plan: &PostgresLoadPlan) -> Result<u64> {
    let (cte, source) = match plan.dedup {
        MergeDedupPolicy::First | MergeDedupPolicy::Last => {
            (merge_dedup_cte(plan), "\"_firn_dedup\"".to_owned())
        }
...
fn merge_dedup_cte(plan: &PostgresLoadPlan) -> String {
    ...
    format!(
        ...
        conflict_columns,
        order_expression(FIRN_SEGMENT_COLUMN, &plan.dedup),
        order_expression(FIRN_ROW_COLUMN, &plan.dedup),
        plan.stage_table.quoted()
    )
}
...
fn insert_load_mirror(
    ...
    let statement = plan
        .mirror_sql
        .iter()
        .find(|statement| statement.name == "record_firn_load")
        .ok_or_else(|| FirnError::internal("Postgres plan missing record_firn_load statement"))?;
    ...
    let xid = receipt
        ...
        .ok_or_else(|| FirnError::internal("Postgres receipt missing xid"))?;
    let duplicate = false;
    ...
    .map_err(|error| postgres_error("insert Postgres _firn_loads mirror", error))?;
    Ok(())
}
...
fn upsert_state_mirror(
    ...
    let statement = plan
        .mirror_sql
        .iter()
        .find(|statement| statement.name == "upsert_firn_state")
        .ok_or_else(|| FirnError::internal("Postgres plan missing upsert_firn_state statement"))?;
    ...
    .map_err(|error| postgres_error("upsert Postgres _firn_state mirror", error))?;
    Ok(())
}
...
fn verify_receipt_with_client(client: &mut Client, receipt: &Receipt) -> Result<()> {
    ...
    let Some(row) = rows.into_iter().next() else {
        return Err(FirnError::destination(
            "receipt is absent from Postgres _firn_loads",
        ));
    };
    let stored = receipt_from_verify_row(row)?;
    if &stored == receipt {
...
fn query_verify_row(tx: &mut Transaction<'_>, receipt: &Receipt) -> Result<Row> {
    ...
    .map_err(|error| postgres_error("query Postgres receipt verification", error))?
    .ok_or_else(|| FirnError::destination("receipt is absent from Postgres _firn_loads"))
}
...
fn verify_parameter(plan: &PostgresLoadPlan, name: &str) -> Result<String> {
    plan.verify
        ...
        .get(name)
        .cloned()
        .ok_or_else(|| FirnError::internal(format!("verify clause missing {name}")))
}
...
fn to_i64(value: u64, name: &str) -> Result<i64> {
    i64::try_from(value).map_err(|_| FirnError::internal(format!("{name} exceeds i64")))
}
...
fn postgres_error(context: impl Into<String>, error: postgres::Error) -> FirnError {
