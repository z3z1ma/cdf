...
pub(crate) fn host_from_url(url: &str) -> Result<String> {
    let (_, rest) = url
        .split_once("://")
        .ok_or_else(|| CdfError::contract("HTTP request URL must include a scheme"))?;
    let authority = rest
        .split(['/', '?', '#'])
        .next()
        .ok_or_else(|| CdfError::contract("HTTP request URL must include a host"))?;
    let host_port = authority.rsplit('@').next().unwrap_or(authority);
    let host = if let Some(stripped) = host_port.strip_prefix('[') {
        stripped
            .split_once(']')
            .map(|(inside, _)| inside)
            .ok_or_else(|| CdfError::contract("IPv6 host is missing closing bracket"))?
    } else {
        host_port.split(':').next().unwrap_or(host_port)
    };
    if host.trim().is_empty() {
        return Err(CdfError::contract("HTTP request URL must include a host"));
    }
