...
impl RetryBudget {
    ...
    pub fn classify_transport_error(message: &str, redactor: &Redactor) -> CdfError {
        CdfError::transient(redactor.redact_text(message))
    }

    pub fn next_retry(&mut self, error: &CdfError, unit: &RetryUnit) -> RetryDecision {
...
impl ErrorKindLabel for CdfError {
    fn kind_label(&self) -> &'static str {
        match self.kind {
