...
pub(crate) fn validate_project_shape(config: &ProjectConfig) -> Result<()> {
    if config.project.name.trim().is_empty() {
        return Err(FirnError::contract("project.name cannot be empty"));
    }
    ...
    if config.environments.is_empty() {
        return Err(FirnError::contract(
            "firn.toml must declare at least one environment",
        ));
    }
    if !config
        .environments
    ...
    if config.resources.is_empty() {
        return Err(FirnError::contract(
            "firn.toml must declare at least one resource source mapping",
        ));
    }
    Ok(())
}
...
pub(crate) fn split_secret_parts(value: &str) -> Result<(&str, &str)> {
    let rest = value
        .strip_prefix("secret://")
        .ok_or_else(|| FirnError::contract("secret reference must use the secret:// scheme"))?;
    let (provider, key) = rest
        .split_once('/')
        .ok_or_else(|| FirnError::contract("secret reference must use secret://provider/key"))?;
    if provider.trim().is_empty() {
        return Err(FirnError::contract("secret provider cannot be empty"));
    }
    Ok((provider, key))
}
...
pub(crate) fn semantic_hash(value: &impl Serialize) -> Result<String> {
    let bytes =
        serde_json::to_vec(value).map_err(|error| FirnError::internal(error.to_string()))?;
