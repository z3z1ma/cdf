...
pub fn parse_firn_toml(input: &str) -> Result<ProjectConfig> {
    let config = toml::from_str::<ProjectConfig>(input)
        .map_err(|error| FirnError::contract(error.to_string()))?;
    validate_project_shape(&config)?;
    Ok(config)
}

pub fn parse_lock(input: &str) -> Result<FirnLock> {
    toml::from_str(input).map_err(|error| FirnError::contract(error.to_string()))
}

pub fn lock_to_toml(lock: &FirnLock) -> Result<String> {
    toml::to_string_pretty(lock).map_err(|error| FirnError::contract(error.to_string()))
}
...
pub fn diff_lockfiles(before: &FirnLock, after: &FirnLock) -> Result<Vec<LockDiff>> {
    let before =
        serde_json::to_value(before).map_err(|error| FirnError::internal(error.to_string()))?;
    let after =
        serde_json::to_value(after).map_err(|error| FirnError::internal(error.to_string()))?;
