...
use cdf_kernel::{CheckpointId, CheckpointStatus, CheckpointStore, PackageHash, RewindRequest};
...
fn assert_commit_requires_receipt_covering_delta<S: CheckpointStore>(store: &S) {
    ...
    wrong_schema.schema_hash = cdf_kernel::SchemaHash::new("other-schema-sha256").unwrap();
...
fn checkpoint_ids(checkpoints: &[cdf_kernel::Checkpoint]) -> Vec<&str> {
    checkpoints
        .iter()
...
mod tests {
    ...
    use cdf_kernel::{
        CdfError, Checkpoint, CheckpointId, PipelineId, Receipt, ResourceId, Result, RewindReport,
        ScopeKey, StateDelta,
    };
    ...
    impl CheckpointStore for FaultyStore {
        ...
        fn commit(&self, checkpoint_id: &CheckpointId, receipt: Receipt) -> Result<Checkpoint> {
            ...
            let checkpoint = state
                ...
                .get(checkpoint_id)
                .cloned()
                .ok_or_else(|| CdfError::contract("missing checkpoint"))?;
            if checkpoint.status != CheckpointStatus::Proposed {
                return Err(CdfError::contract("checkpoint is not proposed"));
            }
            ...
            if !receipt_match.is_valid() && !self.accepts_faulty_receipt(receipt_match) {
                return Err(CdfError::contract("receipt does not cover checkpoint"));
            }
    ...
    impl FaultyStore {
        ...
        fn wrong_rewind_report(&self, request: &RewindRequest) -> Result<RewindReport> {
            ...
            let head = state
                ...
                .cloned()
                .ok_or_else(|| CdfError::contract("missing committed checkpoint"))?;
