...
fn create_schema(conn: &Connection) -> Result<(), CliError> {
    conn.execute_batch(
        ...
        CREATE TABLE package_receipts (
            ...
            upsert_intent INTEGER,
            delete_intent INTEGER,
            hard_deletes INTEGER,
            soft_deletes INTEGER,
            missing_delete_keys INTEGER,
            ignored_deletes INTEGER,
            schema_hash TEXT NOT NULL,
        ...
        let result = (|| -> Result<(), CliError> {
            ...
            insert_receipt
                .execute(params![
                    ...
                    count_kind,
                    optional_to_i64(rows_written)?,
                    optional_to_i64(rows_inserted)?,
                    optional_to_i64(rows_updated)?,
                    optional_to_i64(rows_deleted)?,
                    optional_to_i64(upsert_intent)?,
                    optional_to_i64(delete_intent)?,
                    optional_to_i64(hard_deletes)?,
                    optional_to_i64(soft_deletes)?,
                    optional_to_i64(missing_delete_keys)?,
                    optional_to_i64(ignored_deletes)?,
