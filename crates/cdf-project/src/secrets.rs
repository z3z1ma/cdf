...
impl SecretRef {
    pub fn new(value: impl Into<String>) -> Result<Self> {
        ...
        if key.trim().is_empty() {
            return Err(CdfError::contract("secret key cannot be empty"));
        }
        Ok(Self(value))
    }
...
impl SecretProvider for DefaultSecretProvider {
    fn resolve(&self, uri: &SecretUri) -> Result<SecretValue> {
        match split_secret_uri(uri)?.0 {
            ...
            "keychain" | "os-keychain" | "macos-keychain" => Err(CdfError::auth(format!(
                "secret provider `{}` is not available in this cdf-project build",
                split_secret_uri(uri)?.0
            ))),
            provider => Err(CdfError::auth(format!(
