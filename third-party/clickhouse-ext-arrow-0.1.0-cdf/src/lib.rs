...
use arrow_ipc::convert::IpcSchemaLimits;
...
use clickhouse::{Client, ResponseLimits};
use std::io::Write;
use std::mem;
use std::num::{NonZeroUsize, Saturating};
use std::pin::Pin;
...
pub trait ArrowQueryExt {
    ...
    /// Executes the query with finite HTTP and Arrow IPC allocation ceilings.
    ///
    /// The legacy [`Self::fetch_arrow`] method remains unlimited for backward compatibility.
    fn fetch_arrow_with_limits(self, limits: ArrowStreamLimits) -> Result<ArrowCursor, Error>;
}

impl ArrowQueryExt for Query {
    fn fetch_arrow(self) -> Result<ArrowCursor, Error> {
        fetch_arrow(self, None)
    }

    fn fetch_arrow_with_limits(self, limits: ArrowStreamLimits) -> Result<ArrowCursor, Error> {
        fetch_arrow(self, Some(limits))
    }
}

/// Finite HTTP and Arrow IPC allocation ceilings for one Arrow query stream.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ArrowStreamLimits {
    response: ResponseLimits,
    maximum_message_bytes: NonZeroUsize,
    maximum_body_bytes: NonZeroUsize,
    maximum_record_batch_rows: usize,
    schema_limits: IpcSchemaLimits,
}

impl ArrowStreamLimits {
    /// Creates one set of response, metadata, and body limits.
    pub const fn new(
        response: ResponseLimits,
        maximum_message_bytes: NonZeroUsize,
        maximum_body_bytes: NonZeroUsize,
    ...
    /// Adds a row-count ceiling for each decoded Arrow record batch.
    pub const fn with_max_record_batch_rows(mut self, maximum_rows: usize) -> Self {
        self.maximum_record_batch_rows = maximum_rows;
        self
    }

    /// Adds structural and owned-allocation ceilings for the decoded Arrow schema.
    pub const fn with_schema_limits(
        mut self,
        maximum_nodes: usize,
        ...
        self.schema_limits = IpcSchemaLimits::new(
            maximum_nodes,
...
fn fetch_arrow(query: Query, limits: Option<ArrowStreamLimits>) -> Result<ArrowCursor, Error> {
    let mut query = query
    ...
    if let Some(limits) = limits {
        query = query.with_response_limits(limits.response);
        decoder = decoder
            .with_max_message_size(limits.maximum_message_bytes.get())
            // A decoder may retain one bounded body while creating one equally bounded alignment
            // copy. Limited callers must budget that overlap in their admitted working set.
            .with_max_body_size(limits.maximum_body_bytes.get())
            .with_max_record_batch_rows(limits.maximum_record_batch_rows)
            .with_schema_limits(limits.schema_limits)
            // A compressed output can overlap the encoded body and a second aligned copy. The
            // limited ClickHouse path requests no inner compression and rejects a contradictory
            // compressed batch before allocating decoded buffers.
            .with_reject_compressed_record_batches(true)
            // Arrow dictionaries can accumulate across arbitrarily many messages before a record
            // batch is emitted. ClickHouse's ArrowStream path materializes admitted
            // LowCardinality columns without dictionaries, so the bounded API rejects dictionary
            // messages instead of pretending the per-body limit is a cumulative allocation bound.
            .with_reject_dictionary_batches(true);
    }
    Ok(ArrowCursor {
        cursor: query.fetch_bytes("ArrowStream")?,
