...
impl Response {
    pub(crate) fn new(
        ...
        Self::Waiting(Box::pin(
            collect_response(response, compression, limits).instrument(span),
        ))
    }
...
async fn collect_response(
    ...
    if status == StatusCode::OK && exception_code.is_none() {
        ...
        Ok((
            Chunks::new(response.into_body(), compression, tag, limits),
            summary,
        ))
    } else {
...
async fn collect_bad_response(
    ...
    // Collect incrementally so a malicious peer cannot allocate an unbounded contiguous error
    // body before the caller's admitted response ceiling is checked.
    let maximum_raw_error_body_bytes = limits
        .map(ResponseLimits::maximum_raw_error_body_bytes)
        .unwrap_or(usize::MAX);
    let raw_bytes = match collect_bytes_limited(
        IncomingStream::new(body, limits),
        maximum_raw_error_body_bytes,
        "error body",
    )
    .await
    {
        Ok(collected) => collected,
        Err(error @ Error::ResponseTooLarge { .. }) => return error,
    ...
    let bytes = if compression == Compression::None {
        // Avoid a second contiguous buffer when the bounded raw body is already decoded.
        raw_bytes
    } else {
        ...
        let stream = Decompress::new(stream, compression, limits).map_ok(|chunk| chunk.data);
        let maximum_decoded_error_body_bytes = limits
            .map(ResponseLimits::maximum_decoded_error_body_bytes)
            .unwrap_or(usize::MAX);

        // Raw and decoded bodies coexist here. Callers choose disjoint finite ceilings when they
        // opt in; decompression failure falls back to the already bounded raw body.
        match collect_bytes_limited(
            stream,
            maximum_decoded_error_body_bytes,
            "decoded error body",
        )
        .await
        {
...
async fn collect_bytes_limited(
...
impl Chunks {
    fn new(
        ...
        let stream = IncomingStream::new(stream, limits);
        let stream = Decompress::new(stream, compression, limits);
...
impl IncomingStream {
    fn new(inner: Incoming, limits: Option<ResponseLimits>) -> Self {
        Self {
            inner,
            maximum_chunk_bytes: limits
...
impl Stream for IncomingStream {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let mut incoming = Pin::new(&mut this.inner);

        loop {
            break match incoming.as_mut().poll_frame(cx) {
                Poll::Ready(Some(Ok(frame))) => match frame.into_data() {
                    Ok(bytes) if bytes.len() > this.maximum_chunk_bytes => {
                        Poll::Ready(Some(Err(Error::ResponseTooLarge {
                            stage: "HTTP input chunk",
                            limit: this.maximum_chunk_bytes,
                        })))
                    }
...
impl<S> Decompress<S> {
    fn new(stream: S, compression: Compression, limits: Option<ResponseLimits>) -> Self {
        match compression {
            Compression::None => Self::Plain(stream),
            #[cfg(feature = "lz4")]
            #[allow(deprecated)]
            Compression::Lz4 | Compression::Lz4Hc(_) => Self::Lz4(Lz4Decoder::new(stream, limits)),
            #[cfg(feature = "zstd")]
            Compression::Zstd(_) => Self::Zstd(ZstdHttpDecoder::new(stream, limits)),
        }
    }
}
...
#[tokio::test]
async fn response_collection_admits_exact_split_limit_and_rejects_limit_plus_one() {
    let exact = stream::iter([
        Ok::<_, Error>(Bytes::from_static(b"ab")),
        Ok(Bytes::from_static(b"cd")),
    ]);
    assert_eq!(
        collect_bytes_limited(exact, 4, "test response")
            .await
            .unwrap(),
    ...
    assert!(matches!(
        collect_bytes_limited(oversized, 4, "test response").await,
