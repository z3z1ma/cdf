...
impl CompressionCodec {
    ...
    pub(crate) fn decompress_to_buffer(
        ...
        let buffer = if decompressed_length == 0 {
            ...
            // no compression
            let output = input.slice(LENGTH_OF_PREFIX_DATA as usize);
            if output.len() > maximum_output_bytes {
                return Err(ArrowError::IpcError(format!(
                    "IPC buffer decoded length {} exceeds remaining configured {maximum_output_bytes}-byte limit",
                    output.len()
                )));
            }
            output
        } else if let Ok(decompressed_length) = usize::try_from(decompressed_length) {
            if decompressed_length > maximum_output_bytes {
                return Err(ArrowError::IpcError(format!(
                    "IPC buffer decoded length {decompressed_length} exceeds remaining configured {maximum_output_bytes}-byte limit"
                )));
            }
...
fn decompress_lz4(input: &[u8], decompressed_size: usize) -> Result<Vec<u8>, ArrowError> {
    ...
    if decoder.read(&mut overflow)? != 0 {
        return Err(ArrowError::IpcError(format!(
            "LZ4 IPC buffer expands beyond its declared {decompressed_size}-byte length"
        )));
    }
    Ok(output)
}
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "lz4")]
    fn test_lz4_decompression_rejects_output_beyond_declared_length() {
        let mut compressed = Vec::new();
        super::compress_lz4(b"larger than declared", &mut compressed).unwrap();
        let error = super::decompress_lz4(&compressed, 1).unwrap_err();
        assert!(error.to_string().contains("beyond its declared 1-byte length"));
    }
