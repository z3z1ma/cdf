...
fn read_buffer(
    ...
    maximum_decoded_body_bytes: usize,
) -> Result<Buffer, ArrowError> {
    let start_offset = usize::try_from(buf.offset()).map_err(|_| {
        ArrowError::IpcError(format!(
            "IPC buffer offset {} is negative or not representable",
            buf.offset()
        ))
    })?;
    let length = usize::try_from(buf.length()).map_err(|_| {
        ArrowError::IpcError(format!(
            "IPC buffer length {} is negative or not representable",
            buf.length()
        ))
    })?;
    let end = start_offset
        .checked_add(length)
        .ok_or_else(|| ArrowError::IpcError("IPC buffer range overflowed".to_string()))?;
    if end > a_data.len() {
        return Err(ArrowError::IpcError(format!(
            "IPC buffer range {start_offset}..{end} exceeds {}-byte record body",
            a_data.len()
        )));
    }
    let buf_data = a_data.slice_with_length(start_offset, length);
    let remaining = maximum_decoded_body_bytes
        .checked_sub(*decoded_body_bytes)
        .ok_or_else(|| {
            ArrowError::IpcError("IPC decoded-body accounting exceeded its limit".to_string())
        })?;
    // corner case: empty buffer
    let output = match (buf_data.is_empty(), compression_codec) {
        (true, _) | (_, None) => Ok(buf_data),
        (false, Some(decompressor)) => {
            decompressor.decompress_to_buffer(&buf_data, decompression_context, remaining)
        }
    }?;
    *decoded_body_bytes = decoded_body_bytes
        .checked_add(output.len())
        .ok_or_else(|| {
            ArrowError::IpcError("IPC decoded-body byte count overflowed".to_string())
        })?;
    if *decoded_body_bytes > maximum_decoded_body_bytes {
        return Err(ArrowError::IpcError(format!(
            "IPC decoded buffers retain {} bytes beyond configured {maximum_decoded_body_bytes}-byte limit",
            *decoded_body_bytes
        )));
    }
    Ok(output)
...
fn checked_buffer_prefix(
    ...
    if length > buffer.len() {
        return Err(ArrowError::IpcError(format!(
            "IPC {label} requires {length} bytes but its buffer contains only {} bytes",
            buffer.len()
        )));
    }
    Ok(buffer.slice_with_length(0, length))
}
...
impl RecordBatchDecoder<'_> {
        ...
        match data_type {
            ...
            BinaryView | Utf8View => {
                ...
                let count = usize::try_from(count)
                    ...
                    .ok_or_else(|| {
                        ArrowError::IpcError(format!(
                            "Variadic buffer count overflowed for {data_type} column"
                        ))
                    })?; // view and null buffer.
                let buffers = (0..count)
            ...
            Union(fields, mode) => {
                ...
                let type_id_buffer = self.next_buffer()?;
                let type_ids = self.checked_scalar_buffer::<i8>(
                    type_id_buffer,
                    len,
                    "union type-id buffer",
                )?;

                let value_offsets = match mode {
                    UnionMode::Dense => {
                        let offset_buffer = self.next_buffer()?;
                        let offsets = self.checked_scalar_buffer::<i32>(
                            offset_buffer,
                            len,
                            "dense-union offset buffer",
                        )?;
                        Some(offsets)
                    }
    ...
    fn create_struct_array(
        ...
        let nulls = if null_count > 0 {
            let required_validity_bytes = len.div_ceil(8);
            let validity_buffer = checked_buffer_prefix(
                null_buffer,
                required_validity_bytes,
                "struct validity buffer",
            )?;
            let validity_buffer = BooleanBuffer::new(validity_buffer, 0, len);
...
pub struct RecordBatchDecoder<'a> {
    ...
    /// Cumulative logical bytes admitted across decoded record buffers.
    decoded_body_bytes: usize,
    /// Maximum cumulative logical bytes admitted across decoded record buffers.
    maximum_decoded_body_bytes: usize,
    /// Maximum rows admitted from one record-batch header.
    maximum_record_batch_rows: usize,
    /// Validated record-batch row count.
    batch_length: usize,
    /// The format version
...
impl<'a> RecordBatchDecoder<'a> {
    ...
    fn try_new(
        ...
        let batch_length = usize::try_from(batch.length()).map_err(|_| {
            ArrowError::IpcError(format!(
                "IPC record-batch length {} is negative or not representable",
                batch.length()
            ))
        })?;
        for node in field_nodes {
            let length = usize::try_from(node.length()).map_err(|_| {
                ArrowError::IpcError(format!(
                    "IPC field-node length {} is negative or not representable",
                    node.length()
                ))
            })?;
            let null_count = usize::try_from(node.null_count()).map_err(|_| {
                ArrowError::IpcError(format!(
                    "IPC field-node null count {} is negative or not representable",
                    node.null_count()
                ))
            })?;
            if null_count > length {
                return Err(ArrowError::IpcError(format!(
                    "IPC field-node null count {null_count} exceeds length {length}"
                )));
            }
        }
        for buffer in buffers {
            ...
            if end > buf.len() {
                return Err(ArrowError::IpcError(format!(
                    "IPC buffer range {offset}..{end} exceeds {}-byte record body",
                    buf.len()
                )));
            }
        }
        if let Some(counts) = batch.variadicBufferCounts() {
            ...
            for count in counts {
                let count = usize::try_from(count).map_err(|_| {
                    ArrowError::IpcError(format!(
                        "IPC variadic buffer count {count} is negative or not representable"
                    ))
                })?;
                total = total.checked_add(count).ok_or_else(|| {
                    ArrowError::IpcError("IPC variadic buffer count overflowed".to_string())
                })?;
            }
            if total > buffers.len() {
                return Err(ArrowError::IpcError(format!(
                    "IPC variadic buffers declare {total} data buffers but metadata contains only {} buffers",
                    buffers.len()
                )));
            }
        }
        ...
        Ok(Self {
            ...
            maximum_decoded_body_bytes: usize::MAX,
    ...
    /// Set a cumulative logical decoded-buffer ceiling for this record batch.
    pub fn with_max_decoded_body_size(mut self, maximum_bytes: usize) -> Self {
        self.maximum_decoded_body_bytes = maximum_bytes;
        self
    }

    /// Set a row-count ceiling for this record batch.
    ...
    fn read_record_batch(mut self) -> Result<RecordBatch, ArrowError> {
        if self.batch_length > self.maximum_record_batch_rows {
            return Err(ArrowError::IpcError(format!(
                "IPC record batch has {} rows beyond configured {}-row limit",
                self.batch_length, self.maximum_record_batch_rows
            )));
        }
        ...
        let options = RecordBatchOptions::new().with_row_count(Some(self.batch_length));
        ...
        if let Some(projection) = self.projection {
            ...
            self.validate_fully_consumed(&variadic_counts)?;
    ...
    fn next_buffer(&mut self) -> Result<Buffer, ArrowError> {
        ...
        read_buffer(
            ...
            &mut self.decoded_body_bytes,
            self.maximum_decoded_body_bytes,
        )
    }

    fn checked_scalar_buffer<T: ArrowNativeType>(
        &self,
        buffer: Buffer,
        length: usize,
        label: &str,
    ) -> Result<ScalarBuffer<T>, ArrowError> {
        let byte_length = length
            .checked_mul(std::mem::size_of::<T>())
            .ok_or_else(|| ArrowError::IpcError(format!("IPC {label} length overflowed")))?;
        let buffer = checked_buffer_prefix(buffer, byte_length, label)?;
        ...
        if self.require_alignment {
            return Err(ArrowError::IpcError(format!(
                "IPC {label} is not aligned to {alignment} bytes"
            )));
        }
        let mut aligned = MutableBuffer::new(byte_length);
        aligned.extend_from_slice(&buffer);
        Ok(Buffer::from(aligned).into())
    }

    fn skip_buffer(&mut self) -> Result<(), ArrowError> {
        self.buffers.next().map(|_| ()).ok_or_else(|| {
            ArrowError::IpcError("Buffer count mismatched with metadata".to_string())
        })
    }
    ...
    fn validate_fully_consumed(
        &self,
        variadic_counts: &VecDeque<i64>,
    ) -> Result<(), ArrowError> {
        if !variadic_counts.is_empty() || self.nodes.len() != 0 || self.buffers.len() != 0 {
            return Err(ArrowError::IpcError(format!(
                "IPC record metadata was not consumed exactly: {} variadic counts, {} field nodes, {} buffers remain",
                variadic_counts.len(),
                self.nodes.len(),
                self.buffers.len()
            )));
        ...
        match field.data_type() {
            ...
            Utf8View | BinaryView => {
                ...
                let count = usize::try_from(count)
                    .map_err(|_| {
                        ArrowError::IpcError(format!(
                            "Invalid variadic count {count} for {} column",
                            field.data_type()
                        ))
                    })?
                    .checked_add(2)
                    .ok_or_else(|| {
                        ArrowError::IpcError(format!(
                            "Variadic buffer count overflowed for {} column",
                            field.data_type()
                        ))
                    })?; // view and null buffer.
                for _ in 0..count {
...
mod tests {
    ...
    #[test]
    fn malformed_record_batch_ranges_and_union_prefixes_return_errors() {
        use crate::r#gen::Message::*;
        use flatbuffers::FlatBufferBuilder;
        ...
        let buffers = fbb.create_vector(&[
            crate::Buffer::new(0, 0),
            crate::Buffer::new(-1, 4),
        ]);
        let batch_offset = RecordBatch::create(
            &mut fbb,
            &RecordBatchArgs {
        ...
        fbb.finish_minimal(batch_offset);
        let batch = flatbuffers::root::<RecordBatch>(fbb.finished_data()).unwrap();
        let error = RecordBatchDecoder::try_new(
        ...
        .err()
        .unwrap();
        assert!(error.to_string().contains("buffer offset -1"));
        ...
        .unwrap();
        let error = decoder.read_record_batch().unwrap_err();
        assert!(error.to_string().contains("union type-id buffer requires 1 bytes"));
        ...
        let buffers = fbb.create_vector(&[
            crate::Buffer::new(0, 1),
            crate::Buffer::new(1, 4),
            crate::Buffer::new(5, 0),
            crate::Buffer::new(5, 4),
            crate::Buffer::new(9, 0),
            crate::Buffer::new(9, 0),
        ]);
        let batch_offset = RecordBatch::create(
            &mut fbb,
        ...
        fbb.finish_minimal(batch_offset);
        let body = Buffer::from_vec(vec![0, 0, 0, 0, 0, 42, 0, 0, 0]);
        let batch = flatbuffers::root::<RecordBatch>(fbb.finished_data()).unwrap();
        let error = RecordBatchDecoder::try_new(
            &body,
            batch,
        ...
        .read_record_batch()
        .unwrap_err();
        assert!(error.to_string().contains("not aligned to 4 bytes"));

        let batch = flatbuffers::root::<RecordBatch>(fbb.finished_data()).unwrap();
        let decoded = RecordBatchDecoder::try_new(
            &body,
            batch,
            schema,
            &dictionaries,
    ...
    #[test]
    fn malformed_struct_validity_buffer_returns_error() {
        use crate::r#gen::Message::*;
        use flatbuffers::FlatBufferBuilder;

        let struct_fields = Fields::from(vec![Field::new("value", DataType::Int32, false)]);
        let schema = Arc::new(Schema::new(vec![Field::new(
            "record",
        ...
        let mut fbb = FlatBufferBuilder::new();
        let nodes = fbb.create_vector(&[FieldNode::new(1, 1), FieldNode::new(1, 0)]);
        let buffers = fbb.create_vector(&[
            crate::Buffer::new(0, 0),
            crate::Buffer::new(0, 0),
            crate::Buffer::new(0, 4),
        ]);
        let batch_offset = RecordBatch::create(
            &mut fbb,
            &RecordBatchArgs {
        ...
        fbb.finish_minimal(batch_offset);
        let batch = flatbuffers::root::<RecordBatch>(fbb.finished_data()).unwrap();
        let body = Buffer::from(vec![0_u8; 4]);
        let dictionaries = HashMap::new();
        let decoder = RecordBatchDecoder::try_new(
            &body,
            batch,
    ...
    #[test]
    fn compressed_record_buffer_declared_output_respects_cumulative_limit() {
        use crate::r#gen::Message::*;
        use flatbuffers::FlatBufferBuilder;
        ...
        let buffers = fbb.create_vector(&[
            crate::Buffer::new(0, 0),
            crate::Buffer::new(0, 8),
        ]);
        let compression = BodyCompression::create(
            &mut fbb,
            &BodyCompressionArgs {
                codec: crate::CompressionType::LZ4_FRAME,
                method: crate::BodyCompressionMethod::BUFFER,
            },
        );
        let batch_offset = RecordBatch::create(
            &mut fbb,
        ...
        fbb.finish_minimal(batch_offset);
        let batch = flatbuffers::root::<RecordBatch>(fbb.finished_data()).unwrap();
        let body = Buffer::from_vec(1_024_i64.to_le_bytes().to_vec());
        let dictionaries = HashMap::new();
        let decoder = RecordBatchDecoder::try_new(
            &body,
            batch,
        ...
        .unwrap()
        .with_max_decoded_body_size(16);
        let error = decoder.read_record_batch().unwrap_err();
        assert!(error.to_string().contains("decoded length 1024"));
